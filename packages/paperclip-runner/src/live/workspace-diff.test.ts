...
import { captureThinkingMachWorkspace, diffThinkingMachWorkspace } from "./workspace-diff.js";
...
describe("runner-verified workspace diffs", () => {
  it("captures create, modify, delete, and rename without runner-owned noise", async () => {
    ...
    const before = await captureThinkingMachWorkspace(root);
    ...
    const after = await captureThinkingMachWorkspace(root);
    const diff = diffThinkingMachWorkspace(before, after, "turn-1:workspace");
  ...
  it("returns no record when the workspace did not change", async () => {
    ...
    const before = await captureThinkingMachWorkspace(root);
    const after = await captureThinkingMachWorkspace(root);
    expect(diffThinkingMachWorkspace(before, after, "turn-1:workspace")).toBeNull();
  });
});
