...
const catalogSkills = [
  ...
  {
    id: "thinkingmach:bundled:paperclip-operations:task-planning",
...
describe("teams catalog manifest", () => {
  ...
  it("builds stable manifest entries from catalog team directories", async () => {
    ...
    expect(result.manifest.teams[0]!.requiredSkills).toEqual([
      expect.objectContaining({
        ...
        resolved: true,
        catalogSkillKey: "thinkingmach/bundled/software-development/github-pr-workflow",
        agentSlugs: ["cto"],
      }),
    ]);
  ...
  it("detects stale generated manifests", async () => {
    ...
    expect(result.errors).toContain(
      "generated/catalog.json is stale. Run pnpm --filter @thinkingmach/teams-catalog build:manifest.",
    );
  });
});
