...
mutate — so you can call a tool from a ThinkingMach agent and watch the value
...
the right fixture for showing what ThinkingMach stores versus what the demo
package stores — the package stores the values, ThinkingMach stores the
...
- Four MCP tools (over the Streamable HTTP transport):
  ...
  - `kv_delete` (destructive) — delete a key. Carries `destructiveHint: true`
    so ThinkingMach's catalog quarantines it on first sight.
...
```sh
pnpm --filter @thinkingmach/kv-demo-mcp-server build
...
MCP tools share state, and listens on a fixed loopback port. ThinkingMach's
...
4. If you launched the server with `KV_DEMO_TOKEN`, paste the token in the
   **App key** field. The wizard stores it as an `Authorization: Bearer …`
   header secret.
5. Pick the profile defaults (read/write/destructive) and finish. ThinkingMach
...
- **`remote_http` (recommended for this demo)** — required if you want the
  ...
  from that process. ThinkingMach's `remote_http` gateway forwards every call to
...
- **`local_stdio` (not used here)** — runs MCP servers as supervised child
  processes inside ThinkingMach's runtime slots. Reserved for *trusted local
  deployments* (developer laptop, `local_trusted` or
  `authenticated/private` with `THINKINGMACH_TRUSTED_MCP_RUNTIME_HOST` set on a
  ...
  ship in the ThinkingMach build when you need stdio; do not try to shoehorn
...
## What you should see in ThinkingMach
...
| Concern | Stored in this package | Stored in ThinkingMach |
| --- | --- | --- |
| Key/value entries (`kv_*` data) | In-memory `Map`, lost on restart. | Not stored. ThinkingMach never sees the values directly; the gateway only sees the MCP request/response envelope and the redacted-by-policy view the audit log keeps. |
...
Resetting the demo state usually means resetting this process; ThinkingMach's
...
- **Free the port** — if startup logs `EADDRINUSE`, another `kv-demo`
  ...
  kill <pid>
  ```
- **Quiesce the ThinkingMach side** — disable the connection so the gateway
