...
ThinkingMach and this plugin require **Node 24 or later**. The plugin logs a startup warning when it detects an older host runtime.
...
| `tokenId` / `tokenSecret` | yes | Modal auth tokens. Both must be provided together. ThinkingMach stores pasted values as company secrets. The plugin worker runs in a child process that does not inherit host env vars, so `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET` set on the ThinkingMach server are **not** read by the plugin — provide the tokens in this form. |
