...
here rather than in ThinkingMach core.
...
- opt-in company-scoped ThinkingMach event ingestion controls for issues, comments, and documents; event ingestion is disabled by default and routes captured raw provenance into the default space only
- manual ThinkingMach project/root issue distillation and bounded backfill actions with explicit work items, operation issues, source caps, and estimated cost recording
- ThinkingMach-derived distillation (cursor windows, manual `distill-now`, backfill) always writes into the default wiki space in Phase 1; non-default spaces remain on manual / raw-file ingest until per-space ThinkingMach ingestion profiles ship
- ThinkingMach-derived distillation maintains `wiki/projects/<slug>/standup.md` as the executive current-state view for each represented project, alongside durable `wiki/projects/<slug>/index.md` knowledge pages
...
ThinkingMach-derived text ingestion stays limited to issue titles/descriptions, issue comments, and issue documents.
...
From the ThinkingMach repo root:

```bash
pnpm --filter @thinkingmach/plugin-llm-wiki typecheck
pnpm --filter @thinkingmach/plugin-llm-wiki test
pnpm --filter @thinkingmach/plugin-llm-wiki build
```

## Alpha Verification

Run these commands from the ThinkingMach repo root before handing off alpha plugin
changes:
...
- disabled and enabled ThinkingMach event ingestion paths
...
- Host-level plugin install and live agent invocation still need ThinkingMach
...
The package-local `dist/` directory is ignored by git, but local ThinkingMach
...
restart the ThinkingMach dev server so the host imports the fresh bundle.
...
write the starter files. The plugin uses ThinkingMach's local folder API for path
...
`CLAUDE.md` renamed to `AGENTS.md` and ThinkingMach project overviews, standups,
