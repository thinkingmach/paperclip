...
type ThinkingMachSprite = (typeof THINKINGMACH_SPRITES)[number];
...
function spriteSize(sprite: ThinkingMachSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = THINKINGMACH_SPRITES[(baseCol + baseRow) % THINKINGMACH_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = THINKINGMACH_SPRITES[Math.floor(Math.random() * THINKINGMACH_SPRITES.length)]!;
