...
- a minimal React widget rendered in the ThinkingMach dashboard
...
```bash
pnpm --filter @thinkingmach/plugin-hello-world-example build
npx thinkingmach plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `npx thinkingmach plugin uninstall paperclip.hello-world-example --force` then  
  `npx thinkingmach plugin install ./packages/plugins/examples/plugin-hello-world-example`.
