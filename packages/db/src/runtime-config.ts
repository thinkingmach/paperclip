...
import {
  ...
  resolveThinkingMachEnvPathForConfig,
...
function resolveThinkingMachConfigPath(): string {
  if (process.env.THINKINGMACH_CONFIG?.trim()) {
    return path.resolve(process.env.THINKINGMACH_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveThinkingMachConfigPathForInstance();
}

function resolveThinkingMachEnvPath(configPath: string): string {
  return resolveThinkingMachEnvPathForConfig(configPath);
}
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveThinkingMachEnvPath(configPath);
