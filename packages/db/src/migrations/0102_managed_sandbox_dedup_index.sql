...
-- Keeps the oldest row per (company_id) where driver='sandbox' AND metadata.managedByThinkingMach is true.
-- Scoped to ThinkingMach-managed rows only: tenant-created sandbox envs (without the marker) are untouched.
DO $$
DECLARE
...
BEGIN
  WITH ranked AS (
    ...
    WHERE driver = 'sandbox'
      AND (metadata ->> 'managedByThinkingMach')::boolean = true
  ),
  deleted AS (
