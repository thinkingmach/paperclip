...
Use when:
- You want ThinkingMach to run the native Grok Build CLI locally on the host machine
...
- Grok CLI is not installed or authenticated on the machine that runs ThinkingMach

Core fields:
- cwd (string, optional): default absolute working directory fallback for the agent process (created if missing when possible)
- instructionsFilePath (string, optional): absolute path to a markdown instructions file. ThinkingMach stages it into the execution workspace as \`Agents.md\` when safe, otherwise falls back to \`--rules @file\`
...
- ThinkingMach stages desired runtime skills into \`.claude/skills\` inside the execution workspace so Grok discovers them as project skills.
