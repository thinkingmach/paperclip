import type { AdapterSessionManagement, ServerAdapterModule } from "@thinkingmach/adapter-utils";
...
- ThinkingMach should create Hermes runs through POST /v1/runs and observe them through SSE events.
- You need remote Hermes session continuity with X-Hermes-Session-Key.

Don't use when:
- Hermes should run as a local child process on the ThinkingMach host; use hermes_local instead.
- The Hermes API server is not enabled or is only reachable over an unsafe public HTTP endpoint.
- You need Hermes-originated ThinkingMach task creation only; that is a ThinkingMach skill/API bridge, not this wake adapter.
...
- paperclipApiUrl (string): ThinkingMach API URL reachable from the Hermes host. This is not a credential.
...
- Sends Idempotency-Key equal to the ThinkingMach run id for correlation only; Hermes v0.16.0 did not dedupe duplicate creates.
...
- Do not put Hermes apiKey or ThinkingMach bearer tokens in prompts, comments, logs, or result JSON.
