...
test("root package export exposes ThinkingMach external adapter entrypoint", () => {
...
test("Hermes adapter exposes bundled ThinkingMach task bridge skill", async () => {
  const adapter = createServerAdapter();
...
test("Hermes keeps the operational ThinkingMach skill linked after an empty replacement", async () => {
  ...
  try {
    ...
    await fs.writeFile(path.join(source, "SKILL.md"), "# ThinkingMach\n", "utf8");
    const adapter = createServerAdapter();
    ...
    expect(snapshot?.desiredSkills).toContain("thinkingmach/paperclip/paperclip");
