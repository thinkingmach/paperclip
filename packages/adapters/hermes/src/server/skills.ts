...
async function buildSkillEntry(
  ...
  return {
    ...
    readOnly: true, // Hermes manages its own skills — ThinkingMach can't toggle them
...
async function buildHermesSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  // 1. Scan ThinkingMach-managed skills (bundled with the adapter)
  const paperclipEntries = await readThinkingMachRuntimeSkillEntries(config, __moduleDir);
  const desiredSkills = resolveLegacyThinkingMachDesiredSkillNames(config, paperclipEntries);
  ...
  // 3. Merge: ThinkingMach skills first (ephemeral), then Hermes skills
  const entries: AdapterSkillEntry[] = [];
  const warnings: string[] = [];

  // ThinkingMach-managed skills
  ...
  for (const entry of hermesSkillEntries) {
    // Skip if ThinkingMach already manages a skill with the same key
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(
      `Desired skill "${desiredSkill}" is not available in ThinkingMach or Hermes skills.`,
    );
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      ...
      targetPath: null,
      detail:
        "Cannot find this skill in ThinkingMach or ~/.hermes/skills/.",
    });
  }
...
export async function reconcileHermesThinkingMachSkills(
  ...
  const desiredSkills = requestedDesiredSkills
    ? Array.from(new Set([
        ...resolveLegacyThinkingMachDesiredSkillNames({}, availableEntries),
  ...
  for (const entry of availableEntries) {
    if (!desiredSet.has(entry.key) || isThinkingMachSkillSourceMissing(entry)) continue;
...
export async function syncHermesSkills(
  ...
  await reconcileHermesThinkingMachSkills(ctx.config, desiredSkills);
  return buildHermesSkillSnapshot(ctx.config);
}
