...
const MANAGED_MCP_BLOCK_START = "# BEGIN THINKINGMACH MANAGED MCP";
const MANAGED_MCP_BLOCK_END = "# END THINKINGMACH MANAGED MCP";
...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.THINKINGMACH_IN_WORKTREE ?? "");
}
...
/**
 * True when `homePath` lives under the ThinkingMach-managed company tree
 ...
 * external/user-supplied override that ThinkingMach must not seed or overwrite.
 */
...
export async function ensureSymlink(target: string, source: string): Promise<void> {
  ...
  if (!existing.isSymbolicLink()) {
    // A previous ThinkingMach version copied this file into the managed home
    ...
    // ThinkingMach-managed company home, never the user's real ~/.codex.
    ...
    // a ThinkingMach-written stale copy and warrants operator inspection rather
    // than silent removal.
...
function buildManagedMcpBlock(input: {
  ...
  const lines = [
    ...
    "# Written by ThinkingMach for governed MCP gateway access. Do not edit this block by hand.",
  ];
  input.gateways.forEach((gateway, index) => {
    ...
    if (directOverlap) {
      warnings.push(
        `Found unmanaged Codex MCP server "${gateway.name}" overlapping a ThinkingMach-governed gateway; leaving the direct entry in place and adding managed gateway "${managedName}". ThinkingMach cannot enforce policies for that direct entry.`,
      );
    }
 ...
 *
 * `sourceDir`'s *direct* children are the ThinkingMach-injected skill symlinks that
...
/**
 * Seeds auth/config into an explicit ThinkingMach-managed `targetHome`. Symlinks
...
export async function seedManagedCodexHome(
  ...
  if (seedFromShared) {
    ...
    await onLog(
      "stdout",
      `[paperclip] Using ${isWorktreeMode(env) ? "worktree-isolated" : "ThinkingMach-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
    );
  }
...
export interface CodexCredentialReadiness {
  /** True when ThinkingMach owns the effective home and is responsible for its auth. */
  managed: boolean;
 ...
 *   is always treated as ready (ThinkingMach must not seed or inspect it).
...
export async function evaluateCodexCredentialReadiness(
  ...
  if (!effectiveHomeIsManaged) {
    // Genuine external override: ThinkingMach never seeds or inspects it.
    return {
      managed: false,
