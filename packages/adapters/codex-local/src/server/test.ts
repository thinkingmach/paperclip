...
async function prepareCodexHelloProbe(input: {
  ...
  if (input.probeApiKey) {
    ...
    return {
      ...
      args: [
        "-c",
        'set -e; mkdir -p "$CODEX_HOME"; umask 077; printf "%s" "$_THINKINGMACH_CODEX_AUTH_JSON" > "$CODEX_HOME/auth.json"; unset _THINKINGMACH_CODEX_AUTH_JSON; trap \'rm -rf "$CODEX_HOME"\' EXIT INT TERM; "$0" "$@"',
        input.command,
        ...input.args,
      ],
      env: {
        ...
        _THINKINGMACH_CODEX_AUTH_JSON: JSON.stringify({ OPENAI_API_KEY: input.probeApiKey }),
      },
      cleanup,
    };
  }
        ...
        if (probe.timedOut) {
          ...
          checks.push({
            ...
            hint: probeApiKey
              ...
              : "Codex CLI does not read OPENAI_API_KEY from the environment; set OPENAI_API_KEY in this adapter's config (so ThinkingMach writes it to `$CODEX_HOME/auth.json`) or run `codex login` on the host first.",
          });
          if (targetIsSandbox) {
