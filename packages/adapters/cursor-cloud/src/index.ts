...
Notes:
- ThinkingMach reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each ThinkingMach heartbeat maps to a Cursor run on that durable agent.
- ThinkingMach injects THINKINGMACH_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- ThinkingMach remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
