...
import { asString, runChildProcess } from "@thinkingmach/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.THINKINGMACH_PI_COMMAND === "string" &&
    process.env.THINKINGMACH_PI_COMMAND.trim().length > 0
      ? process.env.THINKINGMACH_PI_COMMAND.trim()
      : "pi";
