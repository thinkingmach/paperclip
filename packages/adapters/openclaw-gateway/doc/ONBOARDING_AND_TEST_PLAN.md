...
## Scope
This plan is now **gateway-only**. ThinkingMach supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses ThinkingMach skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a ThinkingMach task.
...
```bash
pnpm --filter @thinkingmach/server test:run -- openclaw-gateway
pnpm --filter @thinkingmach/server typecheck
pnpm --filter @thinkingmach/ui typecheck
pnpm --filter thinkingmach typecheck
```
