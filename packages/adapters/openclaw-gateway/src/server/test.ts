...
import { asString, parseObject } from "@thinkingmach/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the ThinkingMach server host.",
        });
      }
    } catch (err) {
      checks.push({
