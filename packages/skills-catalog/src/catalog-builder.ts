...
const CATALOG_PACKAGE_NAME = "@thinkingmach/skills-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @thinkingmach/skills-catalog build:manifest.");
    }
  }
...
function skillIdForCandidate(candidate: BaseSkillCandidate) {
  return `thinkingmach:${candidate.kind}:${candidate.category}:${candidate.slug}`;
}
