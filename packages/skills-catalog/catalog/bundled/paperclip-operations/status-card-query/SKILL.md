...
description: Create and maintain agent-authored ThinkingMach status cards, or compile a prose interest prompt into bounded CompanySearchQuery objects and write the first summary from the assigned Summarizer run.
key: thinkingmach/bundled/paperclip-operations/status-card-query
recommendedForRoles:
  - general
  - manager
tags:
...
curl -sS -X PATCH \
  ...
  "$THINKINGMACH_API_BASE/api/status-cards/$STATUS_CARD_ID"

curl -sS -X POST \
  -H "Authorization: Bearer $THINKINGMACH_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"full":false}' \
  "$THINKINGMACH_API_BASE/api/status-cards/$STATUS_CARD_ID/refresh"
```
