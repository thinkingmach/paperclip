 ...
 * and tests all agree on the dedicated ThinkingMach runtime port range and the
...
/**
 * Dedicated, allowlisted port range for managed ThinkingMach runtime APP
...
/**
 * Fixed offset between an app port and its ThinkingMach Vite HMR companion port.
 * Matches ThinkingMach dev mode's `server port + offset` HMR convention and keeps
...
export function deriveViteHmrPort(appPort: number): number {
  ...
  return deriveThinkingMachViteHmrPort(appPort);
}

/**
 * Derive ThinkingMach's Vite HMR companion port for any valid application port.
 *
 * Normal ThinkingMach instances can use ports outside the dedicated exposure
 ...
 */
export function deriveThinkingMachViteHmrPort(serverPort: number): number {
