# ThinkingMach Daytona runner image

This image is the ThinkingMach Cloud fleet sandbox image plus a source-built
...
[`paperclip-cloud/fleet-sandbox-image/Dockerfile`](https://github.com/thinkingmach/paperclip-cloud/blob/master/fleet-sandbox-image/Dockerfile).
...
docker buildx build \
  ...
  --build-arg THINKINGMACH_RUNNER_SOURCE_REVISION="$(git rev-parse HEAD)" \
...
`THINKINGMACH_RUNNER_SOURCE_REVISION` remains the full Git SHA that built the first
...
environment image to that immutable tag or snapshot. ThinkingMach probes the
...
`THINKINGMACH_RUNNER_REMOTE_PROVIDER_PACK_PATH`. Remote OpenCode and ACPX never
...
full Git SHA as `THINKINGMACH_RUNNER_SOURCE_REVISION`.

Do not bake provider credentials, ThinkingMach bootstrap tickets, or Daytona
