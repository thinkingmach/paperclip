...
ClipHub is the public registry where people share, discover, and download ThinkingMach company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to ThinkingMach what a package registry is to a programming language. ThinkingMach already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in ThinkingMach — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own ThinkingMach instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the ThinkingMach spec. It contains:
...
```
Downloads the template and creates a new company in your local ThinkingMach instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within ThinkingMach, export your company as a template, then publish:
...
ClipHub is a **separate service** from ThinkingMach itself. ThinkingMach is self-hosted; ClipHub is a hosted registry that ThinkingMach instances talk to.
...
| **ThinkingMach CLI** | `thinkingmach install`, `thinkingmach publish`, `thinkingmach cliphub sync` — built into ThinkingMach |
| **ThinkingMach UI** | "Browse ClipHub" panel in the ThinkingMach web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with ThinkingMach) |
| Backend | TypeScript + Hono (consistent with ThinkingMach) |
| Database | PostgreSQL |
...
4. Run `thinkingmach install cliphub:acme/lean-saas-shop`
5. ThinkingMach creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in ThinkingMach until it works well
2. Export: `thinkingmach export --template my-agency`
3. Publish: `thinkingmach publish cliphub my-agency`
...
3. Install just that agent: `thinkingmach install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to ThinkingMach

ClipHub is **not required** to use ThinkingMach. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to ThinkingMach what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `thinkingmach install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in ThinkingMach web UI
- [ ] `thinkingmach cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with ThinkingMach's own philosophy)
