# ThinkingMach Specification

Target specification for the ThinkingMach control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One ThinkingMach instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the ThinkingMach protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

ThinkingMach doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### ThinkingMach Protocol (What ThinkingMach Knows)

At the protocol level, ThinkingMach tracks:
...
- **Fat payload** — ThinkingMach bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to ThinkingMach.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls ThinkingMach's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a ThinkingMach agent: **be callable.** That's it. ThinkingMach can invoke you via command or webhook. No requirement to report back — ThinkingMach infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, ThinkingMach provides progressively richer integration:

1. **Callable** (minimum) — ThinkingMach can start you. That's the only contract.
...
ThinkingMach ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **ThinkingMach skill** (a Claude Code skill for interacting with the ThinkingMach API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the ThinkingMach SKILL.md which is recommended for all agents. ThinkingMach the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. ThinkingMach defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how ThinkingMach invokes the agent. Built-in adapters include:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets ThinkingMach check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the ThinkingMach REST API.

### What ThinkingMach Controls
...
### What ThinkingMach Does NOT Control
...
- Health monitoring — how does ThinkingMach distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of ThinkingMach. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to ThinkingMach. Costs are tracked at every level:
...
ThinkingMach ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **ThinkingMach Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### ThinkingMach Skill (SKILL.md)

A skill definition that teaches agents how to interact with ThinkingMach. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my ThinkingMach instance."

#### Agent Authentication

When a user creates an Agent, ThinkingMach generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. ThinkingMach generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), ThinkingMach sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the ThinkingMach SKILL, but sometimes it's unavoidable.
...
When an agent crashes or disappears mid-task, ThinkingMach does **not** auto-reassign or auto-release the task. Instead:

- ThinkingMach surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- ThinkingMach may perform bounded continuity repair with the same assigned agent; when that is exhausted or unsafe, it opens a board-owned recovery action without waking a substitute agent
- ThinkingMach does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: ThinkingMach reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core ThinkingMach system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`thinkingmach plugin`) are available for authoring and installing plugins.
...
**Full loop with one adapter.** V1 must demonstrate the complete ThinkingMach cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with ThinkingMach skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **ThinkingMach skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the ThinkingMach core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things ThinkingMach explicitly does **not** do:

- **Not an Agent runtime** — ThinkingMach orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. ThinkingMach is the control plane, not the execution plane.
