...
Run ThinkingMach in Docker without installing Node or pnpm locally.
...
docker run --name paperclip \
  ...
  -e THINKINGMACH_TOOL_ACTION_SIGNING_SECRET=$(openssl rand -hex 32) \
...
THINKINGMACH_TOOL_ACTION_SIGNING_SECRET=$(openssl rand -hex 32) \
...
**Note:** `THINKINGMACH_DATA_DIR` is resolved relative to the compose file (`docker/`), so `../data/pc` maps to `data/pc` in the project root.

If you change host port or use a non-local domain, set `THINKINGMACH_PUBLIC_URL` to the external URL you will use in browser/auth flows.

Pass `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` to enable local adapter runs.

### Full stack (with PostgreSQL)

ThinkingMach server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). ThinkingMach data persists in `paperclip-data`.
...
For authenticated deployments, set one canonical public URL and let ThinkingMach derive auth/callback defaults:
...
services:
  paperclip:
    environment:
      ...
      THINKINGMACH_PUBLIC_URL: https://desk.koker.net
```

`THINKINGMACH_PUBLIC_URL` is used as the primary source for:
...
ThinkingMach URL, sign in or create an account, then choose `Claim this instance`
...
Granular overrides remain available if needed (`THINKINGMACH_AUTH_PUBLIC_BASE_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, `THINKINGMACH_ALLOWED_HOSTNAMES`).

Set `THINKINGMACH_ALLOWED_HOSTNAMES` explicitly only when you need additional hostnames beyond the public URL host (for example Tailscale/LAN aliases or multiple private hostnames).
...
`THINKINGMACH_VERCEL_CONNECT_ENABLED=true` does not expose a customer-facing setup
...
can provide `THINKINGMACH_VERCEL_CONNECT_ACCESS_TOKEN` as a deployment bootstrap
...
services:
  paperclip:
    environment:
      ...
      THINKINGMACH_VERCEL_CONNECT_ACCESS_TOKEN: ${THINKINGMACH_VERCEL_CONNECT_ACCESS_TOKEN}
```
...
- Adapter environment checks in ThinkingMach will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run ThinkingMach + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/paperclip.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/paperclip.container` | ThinkingMach server — joins the pod, connects to Postgres at `127.0.0.1` |
...
- Containers in a pod share `localhost`, so ThinkingMach reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `paperclip-pgdata` named volume.
- ThinkingMach data persists at `~/.local/share/paperclip`.
...
- `npx thinkingmach onboard --yes` completes
...
```sh
HOST_PORT=3200 THINKINGMACH_VERSION=latest ./scripts/docker-onboard-smoke.sh
THINKINGMACH_DEPLOYMENT_MODE=authenticated THINKINGMACH_DEPLOYMENT_EXPOSURE=private ./scripts/docker-onboard-smoke.sh
SMOKE_DETACH=true SMOKE_METADATA_FILE=/tmp/paperclip-smoke.env THINKINGMACH_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- Smoke script defaults host port to `3131` to avoid conflicts with local ThinkingMach on `3100`.
- Smoke script also defaults `THINKINGMACH_PUBLIC_URL` to `http://localhost:<HOST_PORT>` so bootstrap invite URLs and auth callbacks use the reachable host port instead of the container's internal `3100`.
- In authenticated mode, the smoke script defaults `SMOKE_AUTO_BOOTSTRAP=true` and drives the real bootstrap path automatically: it signs up a real user, runs `thinkingmach auth bootstrap-ceo` inside the container to mint a real bootstrap invite, accepts that invite over HTTP, and verifies board session access.
...
- ThinkingMach data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/paperclip` (quadlet).
