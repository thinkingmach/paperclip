...
Low-level reference for how ThinkingMach packages are prepared and published to npm.
...
ThinkingMach no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `thinkingmach`, imports code from workspace packages such as:
...
### Manual first publish for `@thinkingmach/ui`
...
npm view @thinkingmach/ui version
...
- If `npm view @thinkingmach/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@thinkingmach/ui`, the `thinkingmach` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
ThinkingMach uses calendar versions:
...
- `thinkingmach@2026.318.1-canary.2`
...
- every published internal `@thinkingmach/*` dependency referenced by that manifest exists on npm
...
ThinkingMach does not auto-publish every non-private workspace package anymore.
...
pnpm run release:bootstrap-package -- @thinkingmach/new-package
...
pnpm run release:bootstrap-package -- @thinkingmach/new-package --publish
```

The helper script:

- refuses names outside the `@thinkingmach/` scope
...
1. open `https://www.npmjs.com/package/@thinkingmach/new-package`
2. go to `Settings` → `Trusted publishing`
3. add repository `thinkingmach/paperclip`
