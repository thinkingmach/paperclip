# ThinkingMach Runner in Daytona sandboxes

## Intended topology

ThinkingMach creates the run and a short-lived, one-use runner bootstrap ticket. The Daytona sandbox starts only `paperclip-runnerd`; runnerd then starts the configured provider (Codex initially). Runnerd listens on a fixed sandbox-local port and ThinkingMach connects through Daytona's authenticated preview WebSocket ingress. The provider never receives a ThinkingMach API credential or Daytona preview credential.

```text
ThinkingMach control plane
       |
...
- Allow ThinkingMach to make outbound TCP 443 connections to the configured Daytona preview origin. The ThinkingMach host does not need public ingress.
...
- Use Daytona's `wss://` preview URL with normal certificate and hostname validation. The preview token is sent only as the `X-Daytona-Preview-Token` header by ThinkingMach.
...
1. ThinkingMach allocates the Daytona environment and persists its environment lease.
2. ThinkingMach creates a runner ticket with a very short expiry and binds it to the run, environment lease, runner digest/version, and allowed catalog digest.
...
4. ThinkingMach acquires an authenticated Daytona preview endpoint and dials runnerd. After PRP authentication, ThinkingMach sends `run.prepare`; only then may runnerd start Codex and advertise the run-authorized tools.
5. Suspend/drain/revoke commands stop new turns and durably flush terminal events. ThinkingMach revokes the lease before destroying or recycling the sandbox.
...
`wss://` only when an operator explicitly configures a reachable ThinkingMach
