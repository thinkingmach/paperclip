...
Audience: ThinkingMach operators and engineers enabling Vercel Connect for outbound
remote MCP apps.

Vercel Connect is an optional credential authority inside Apps v2. Operators
create, attach, authorize, and inspect connectors in Vercel. ThinkingMach records
...
not enter ThinkingMach secrets, connection config, agent config, prompts, API
...
> **Preview UI paused:** ThinkingMach currently withholds Vercel Connect from Apps
...
ThinkingMach pins `@vercel/connect` to `0.6.1`.

| Setting | Meaning |
| --- | --- |
| `THINKINGMACH_VERCEL_CONNECT_ENABLED=true` | Enables the backend capability for controlled testing and existing connections. It does not currently expose a customer-facing setup entry. |
| `VERCEL_OIDC_TOKEN` | Workload identity injected by Vercel and preferred when present. |
| `THINKINGMACH_VERCEL_CONNECT_ACCESS_TOKEN` | BYO instance bootstrap authority for deployments without Vercel workload OIDC. |
| `THINKINGMACH_INSTANCE_ID` | Included in derived pseudonymous user subjects when configured. |
...
Treat `THINKINGMACH_VERCEL_CONNECT_ACCESS_TOKEN` like any other deployment
bootstrap secret: inject it through the host/container secret facility, rotate
it outside ThinkingMach, and never put it in `company_secrets`, app config, task
...
rollout. ThinkingMach prefers workload OIDC whenever both authorities are present,
...
normal SDK cache path rather than persisted by ThinkingMach.
...
1. Open [Vercel Connect](https://vercel.com/connect), create the provider
   ...
   ThinkingMach. Vercel remains the inventory and management UI.
2. In a future reviewed build, ThinkingMach will expose one isolated Vercel Connect
...
4. ThinkingMach calls `getConnectorMetadata` and rejects missing, unattached, or
...
5. For a user OAuth connector, continue through Vercel authorization. The
   ...
   app/API-key connectors, ThinkingMach verifies setup with a token request. If an
   installation is required, complete it in Vercel and retry; ThinkingMach does not
   use the experimental installation API.
6. Review the discovered catalog and complete the existing ThinkingMach access,
   resource-filter, risk, and agent-install steps.

V1 requires one dedicated app-subject connector per ThinkingMach connection. Do
not reuse an app-subject connector across ThinkingMach companies. Multi-installation
routing and shared app-subject connectors are intentionally deferred.

## Runtime and subject binding

ThinkingMach selects the effective connection grant before requesting a token. It
derives user-subject IDs from the ThinkingMach instance, company, connection, grant
kind, and responsible user. Organization OAuth uses a company-scoped
pseudonymous user subject; personal OAuth adds the ThinkingMach user. App/API-key
methods use Vercel's `app` subject. Neither browsers nor agents may supply these
subject identifiers.

Token resolution uses the SDK cache plus ThinkingMach single-flight deduplication.
...
`none` and PKCE `S256`; ThinkingMach sends the canonical resource plus reviewed
...
placement. ThinkingMach injects the bearer only after the connection grant,
...
Stable ThinkingMach reason codes include:

| Code | Operator action |
| --- | --- |
| `vercel_connect_unavailable` | Restore workload OIDC or the BYO access token. |
| `vercel_connect_auth_failed` | Repair or refresh ThinkingMach's Vercel authority, and verify that the configured token type is accepted by Connect. |
...
ThinkingMach creates the existing authorization interaction. Reconnect UI sends
...
external cleanup. ThinkingMach revokes subjects it generated for user-mode
...
synchronization, ThinkingMach-hosted relay services, and Vercel's experimental
...
instead of copying rates into ThinkingMach documentation.
