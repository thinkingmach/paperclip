...
For `claude_local` and `codex_local`, ThinkingMach assumes the CLI is already installed and authenticated on the host machine.
...
ThinkingMach stores resumable session state per `(agent, taskKey, adapterType)`.
