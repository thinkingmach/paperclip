...
This spec defines how ThinkingMach actually runs agents while staying runtime-agnostic.
...
1. ThinkingMach is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, ThinkingMach manages lifecycle, and on exit ThinkingMach parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared ThinkingMach skill text block)
...
2. Preferred transport is env vars (`THINKINGMACH_*`) injected at runtime.
