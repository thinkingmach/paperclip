...
`PAP-447` asks how ThinkingMach should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same ThinkingMach instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into ThinkingMach:

- some operators use ThinkingMach to manage ThinkingMach and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as ThinkingMach
- Claude and Codex expose different built-in affordances, so ThinkingMach should not overfit to one tool

## Core Product Decision

ThinkingMach should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by ThinkingMach
...
- ThinkingMach should not assume that every adapter can see or use a host filesystem path directly
...
If ThinkingMach makes worktrees agent-first, it will blur:
...
- ThinkingMach core resolves and records execution workspace state
...
That gives ThinkingMach room to support:
...
- ThinkingMach handles workspace runtime services under the hood using project/workspace policy
...
Once ThinkingMach is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So ThinkingMach should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that ThinkingMach cannot infer
...
- when issue moves to done, should ThinkingMach auto-commit?
- should it auto-push?
...
ThinkingMach already has the right foundation for a project-first model.
...
For server-spawned local adapters, ThinkingMach already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that ThinkingMach should mirror directly

Implication:

- for `codex_local`, ThinkingMach should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but ThinkingMach should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, ThinkingMach should separate:
...
ThinkingMach should be able to express intentions such as:
...
These are not specific to ThinkingMach. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a ThinkingMach-specific `devServer` flag.
...
ThinkingMach should distinguish between:
...
ThinkingMach should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, ThinkingMach should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to ThinkingMach
...
For runtime services specifically, ThinkingMach should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as ThinkingMach-only dev-server flags
...
- same embedded DB / ThinkingMach instance can be reused through env/config injection when appropriate
...
- ThinkingMach can decide whether to reuse or start a fresh service deterministically
...
3. Test against ThinkingMach-on-ThinkingMach workflows first.
...
7. Workspace runtime services are modeled generically, not as ThinkingMach-specific dev-server toggles.
