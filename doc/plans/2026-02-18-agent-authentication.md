...
Agents need API keys to authenticate with ThinkingMach. The current approach
...
**Trust model:** The adapter process runs on the same machine as the ThinkingMach
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** ThinkingMach generates a token and passes it directly to the agent
...
`THINKINGMACH_API_URL`, `THINKINGMACH_AGENT_ID`, `THINKINGMACH_COMPANY_ID`. We need to
add a `THINKINGMACH_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a ThinkingMach CLI
...
managed by the ThinkingMach server itself.
...
1. A company admin (user or agent) generates an **invite URL** from ThinkingMach.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The ThinkingMach SKILL.md (or a link to it)
   - What information ThinkingMach needs from the agent (e.g. webhook URL, adapter
...
5. ThinkingMach stores the pending registration.
...
7. On approval, ThinkingMach provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** ThinkingMach issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.thinkingmach.com/skills/paperclip/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How ThinkingMach should send you heartbeats",
...
1. Generate a ThinkingMach invite link for the company.
...
4. A ThinkingMach company member approves the new agent.
5. ThinkingMach begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `THINKINGMACH_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `thinkingmach auth login` for developer-managed remote agents.                                      |
