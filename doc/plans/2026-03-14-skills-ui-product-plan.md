...
This document defines the product and UI plan for skill management in ThinkingMach.
...
- ThinkingMach company import/export can include skills as package content
...
3. The UI must always tell the truth about what ThinkingMach knows:
   - desired state in ThinkingMach
...
ThinkingMach should treat skills at two scopes:
...
- created locally inside ThinkingMach later
...
7. “Understand what ThinkingMach knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a ThinkingMach-specific package layout
...
This matters because Codex and similar adapters may already have local skills that ThinkingMach did not install.

These should be clearly marked:

- external
- not managed by ThinkingMach
...
- `desired_only`: ThinkingMach wants it, adapter does not show it yet
- `external`: adapter has it but ThinkingMach does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, ThinkingMach only tracks desired state
...
- ThinkingMach can still track desired skills
...
- ThinkingMach can see adapter skills
...
- `.paperclip.yaml` may add ThinkingMach-specific fidelity, but should not replace the base shortname association model
...
- `.paperclip.yaml` may add ThinkingMach-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
ThinkingMach should have a company-scoped skill model or managed package model representing:
...
ThinkingMach should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too ThinkingMach-specific.
...
That gives ThinkingMach one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
