...
This repo already uses `THINKINGMACH_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `THINKINGMACH_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - ThinkingMach may create host-local worktrees, processes, and paths
- `cloud`
  - ThinkingMach should assume no durable host-local execution workspace management
...
- use `THINKINGMACH_EXECUTION_TOPOLOGY` as a defaulting guardrail
