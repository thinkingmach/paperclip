...
- New config/env settings:
  ...
  - `THINKINGMACH_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `THINKINGMACH_AGENT_JWT_ISSUER` (default: `paperclip`)
  - `THINKINGMACH_AGENT_JWT_AUDIENCE` (default: `paperclip-api`)
...
1. In:
   ...
   inject `THINKINGMACH_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `THINKINGMACH_API_KEY`, do not overwrite it.
- Continue injecting:
...
- Local adapters authenticate without manual `THINKINGMACH_API_KEY` config.
