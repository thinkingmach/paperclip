...
ThinkingMach should not replace its workspace runtime service model with VS Code tasks.
...
- ThinkingMach owns long-running workspace services and their desired state
- ThinkingMach shows operators exactly which named thing they are starting or stopping
- ThinkingMach distinguishes long-running services from one-shot jobs
...
This is directionally correct for ThinkingMach because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and ThinkingMach does not yet leverage existing workspace metadata.

## Recommendation

Keep ThinkingMach runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in ThinkingMach.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to ThinkingMach service or job semantics should become runnable in ThinkingMach.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than ThinkingMach runtime services.
...
That creates a bad fit if ThinkingMach tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace ThinkingMach's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

ThinkingMach should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command ThinkingMach can resolve
...
- selected variable substitution when ThinkingMach can resolve it safely from workspace context
- mapping task metadata into ThinkingMach readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types ThinkingMach cannot execute directly
...
ThinkingMach needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by ThinkingMach metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by ThinkingMach
...
- source: ThinkingMach or VS Code
...
For a workspace with `cwd`, ThinkingMach should look for `.vscode/tasks.json`.
...
ThinkingMach should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - ThinkingMach metadata marks it as a service, or
...
ThinkingMach-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what ThinkingMach would import and why.
...
Goal:
run supported imported service commands through the current ThinkingMach supervisor.
...
Success condition:
imported service commands behave like native ThinkingMach services once adopted.
...
- removal of ThinkingMach's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting ThinkingMach to VS Code and making the model worse for non-VS-Code repos
...
ThinkingMach should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing ThinkingMach workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
