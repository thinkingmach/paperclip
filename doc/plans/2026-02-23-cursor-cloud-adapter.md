...
This document defines the V1 design for a ThinkingMach adapter that integrates with
...
4. synthesized stdout events for ThinkingMach UI/CLI
...
3. **Agent callback auth to ThinkingMach** uses a bootstrap exchange flow (no long-lived ThinkingMach key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from ThinkingMach endpoints, not full SKILL.md prompt injection.
...
- `paperclipPublicUrl` (optional override; else `THINKINGMACH_PUBLIC_URL` env)
...
## ThinkingMach Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `THINKINGMACH_API_KEY`.
...
2. `process.env.THINKINGMACH_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived ThinkingMach credentials in prompt text.

Flow:

1. Before launch/follow-up, ThinkingMach mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. ThinkingMach validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all ThinkingMach API calls.
...
1. Prompt includes a compact instruction to fetch skills from ThinkingMach.
...
4. Route by Cursor `agentId` to active ThinkingMach run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from ThinkingMach skill endpoints |
| ThinkingMach auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from ThinkingMach context.
3. Add richer PR metadata surfacing in ThinkingMach UI.
