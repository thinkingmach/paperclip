...
Add a single storage subsystem for ThinkingMach that supports:
...
- API serves attachment bytes through authenticated ThinkingMach endpoints.
...
- `thinkingmach onboard` writes a valid `storage` config block by default.
- `thinkingmach configure --section storage` can switch between local and s3 modes.
...
- `thinkingmach doctor` reports actionable storage status.
