...
Date: 2026-05-26
Source plan: approved ThinkingMach skills CLI and catalog plan

This document freezes the first implementation contract for the `thinkingmach skills`
command group and the app-shipped skills catalog. It is intentionally a build
contract, not a full product spec.

## Decisions

- `thinkingmach skills` manages ThinkingMach company skills. It does not manage
...
- Root `skills/` remains reserved for ThinkingMach runtime and operational skills.
...
- Catalog skill: an app-shipped skill entry in `@thinkingmach/skills-catalog`.
...
- Company-scoped commands also accept `-C, --company-id <id>` and otherwise use
  `THINKINGMACH_COMPANY_ID` or the active context profile.
...
| `skills agent sync <agent-ref> --skill <skill-ref>...` | Replaces the agent's non-required desired skill set with the supplied refs and triggers adapter sync. Required ThinkingMach skills remain enforced by the server. | `AgentSkillSnapshot` |
...
Catalog commands are for the app-shipped ThinkingMach catalog only. External GitHub,
...
Package name: `@thinkingmach/skills-catalog`.
...
```text
thinkingmach:<kind>:<category>:<slug>
```
...
- `Company ID is required. Pass --company-id, set THINKINGMACH_COMPANY_ID, or set a context profile.`
...
- `thinkingmach skills --help` exposes the Phase B command group.
