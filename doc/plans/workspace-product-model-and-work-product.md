...
ThinkingMach needs to support two very different but equally valid ways of working:
...
Today, ThinkingMach already has the beginnings of this model:
...
- making ThinkingMach feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing ThinkingMach users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted ThinkingMach deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning ThinkingMach into a full code review product
...
This object must be recorded explicitly so that ThinkingMach can:
...
ThinkingMach should treat PRs as a type of work product linked back to:
...
ThinkingMach already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted ThinkingMach must be a first-class deployment mode

ThinkingMach cannot assume that it is running on the same machine as the code.

In cloud deployments, ThinkingMach may:
...
- `Runtime service`: a process or service ThinkingMach owns or tracks for a workspace
...
It also lets cloud-hosted ThinkingMach deployments point at codebases and remotes without pretending the ThinkingMach host has direct filesystem access.
...
This lets ThinkingMach support:
...
Without an explicit `execution workspace` record, ThinkingMach has nowhere stable to attach:
...
ThinkingMach needs a single place to show:
...
- ThinkingMach-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without ThinkingMach owning the execution host
...
## 1. ThinkingMach host and execution host must be decoupled

ThinkingMach may run:
...
without the ThinkingMach host itself running local git or local preview processes.
...
ThinkingMach should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for ThinkingMach.
...
ThinkingMach should keep workspace automation centered on a small set of hooks:
...
ThinkingMach should provide the equivalent visibility in the project `Code` surface:
...
ThinkingMach should treat execution workspaces as the natural home for:
...
ThinkingMach should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
ThinkingMach should continue treating cleanup policy as part of the core workspace model:
...
ThinkingMach should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the ThinkingMach host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

ThinkingMach should borrow Worktrunk's good execution patterns while keeping the broader ThinkingMach model:
...
This is how ThinkingMach supports:
...
That keeps ThinkingMach focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted ThinkingMach deployments remain first-class

That is a better fit for ThinkingMach than either extreme:
