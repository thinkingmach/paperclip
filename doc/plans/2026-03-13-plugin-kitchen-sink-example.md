...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented ThinkingMach plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside ThinkingMach.
...
- package: `@thinkingmach/plugin-kitchen-sink-example`
...
- subscribe to a core ThinkingMach event such as `issue.created`
...
Recommendation: default to a ThinkingMach-local endpoint or a stable public echo endpoint to avoid flaky docs.
