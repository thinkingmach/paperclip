...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into ThinkingMach.
...
- current ThinkingMach plugin and adapter surfaces in this repo
...
- ThinkingMach should copy only the safe subset:
  - typed event subscriptions
  - read-only run lifecycle events
  - explicit worker lifecycle hooks
  - plugin-to-plugin events
- ThinkingMach should not copy the dangerous subset:
...
## What ThinkingMach already has

ThinkingMach already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to ThinkingMach

These ideas from `pi-mono` fit ThinkingMach with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

ThinkingMach should continue exposing run and transcript events to plugins, for example:
...
This matches ThinkingMach's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

ThinkingMach already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

ThinkingMach already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in ThinkingMach, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for ThinkingMach core:

### 1. Arbitrary mutation hooks on control-plane decisions

ThinkingMach should not let general plugins rewrite:
...
ThinkingMach should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

ThinkingMach is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to ThinkingMach's board-and-issues model.

## Recommended ThinkingMach direction
...
If the question is "what works for ThinkingMach too?":
