...
This document defines the next-stage plan for ThinkingMach company import/export.

The core shift is:

- move from a ThinkingMach-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside ThinkingMach.
...
ThinkingMach already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just ThinkingMach
...
7. an always-emitted `.paperclip.yaml` sidecar for high-fidelity ThinkingMach-specific details
...
11. adapter-aware skill sync surfaces so ThinkingMach can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point ThinkingMach at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a ThinkingMach sidecar.
...
ThinkingMach must not redefine `SKILL.md`.
...
- ThinkingMach-specific fidelity lives in `.paperclip.yaml`
- ThinkingMach may resolve and install `SKILL.md` packages, but it must not require a ThinkingMach-only skill format
...
### 5.4 Base Package Vs ThinkingMach Extension
...
- base package:
  ...
  - no ThinkingMach-only runtime fields by default
- ThinkingMach extension:
  - `.paperclip.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by ThinkingMach tools as a sidecar while the base package stays readable
...
ThinkingMach import/export should support these entity kinds:
...
In ThinkingMach V2 portability:
...
- ThinkingMach can treat “this set of agents came from team package X” as the imported-team model
...
ThinkingMach should support source references in package metadata with:
...
ThinkingMach should define a new adapter surface area around skills:
...
Planned ThinkingMach behavior:

- if an adapter supports read, ThinkingMach should show current skills in the UI
- if an adapter supports write, ThinkingMach should let the user enable/disable imported skills
- if an adapter supports sync, ThinkingMach should compute desired vs actual state and offer reconcile actions
...
## 10. Storage Model Inside ThinkingMach
...
ThinkingMach should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `thinkingmach company export <company-id> --out <path>`
- `thinkingmach company import <path-or-url> --dry-run`
- `thinkingmach company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in ThinkingMach storage, or only referenced at import time?
...
3. Should ThinkingMach support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps ThinkingMach aligned with:
