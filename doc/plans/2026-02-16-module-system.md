...
ThinkingMach's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends ThinkingMach's API, UI, and data model | Yes |
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@thinkingmach/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@thinkingmach/core";
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@thinkingmach/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@thinkingmach/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "templates": [
    {
      ...
      "url": "https://store.thinkingmach.com/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm thinkingmach store list                    # browse available modules and templates
pnpm thinkingmach store install <module-id>     # install a module
pnpm thinkingmach store import <template-id>    # import a company template
pnpm thinkingmach store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one ThinkingMach instance | `server:started` |
...
10. **`@thinkingmach/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
