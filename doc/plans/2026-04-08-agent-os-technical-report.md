# Agent OS Technical Report for ThinkingMach
...
`agent-os` is not a competitor to ThinkingMach's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. ThinkingMach is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that ThinkingMach could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in ThinkingMach.

My recommendation is:

1. Do not merge agent-os concepts into the ThinkingMach core product model.
...
4. Do not import its workflow/cron/queue abstractions into ThinkingMach core until they are reconciled with ThinkingMach's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than ThinkingMach's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to ThinkingMach because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## ThinkingMach anchor points

The most relevant current ThinkingMach surfaces for any future `agent-os` integration are:
...
## What ThinkingMach can learn from it

### 1. A safer local execution substrate

ThinkingMach's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits ThinkingMach because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

ThinkingMach already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
ThinkingMach has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that ThinkingMach could adopt even without adopting the runtime itself:
...
ThinkingMach's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.

Concrete ThinkingMach uses:
...
This aligns well with ThinkingMach's governance story.

### 5. Better adapter normalization around sessions and capabilities

ThinkingMach's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
ThinkingMach does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
ThinkingMach can adopt that philosophy directly:
...
## What does not fit ThinkingMach well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. ThinkingMach already has higher-level orchestration concepts:
...
If ThinkingMach copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

ThinkingMach should keep orchestration authoritative at the control-plane layer.
...
Those are ThinkingMach's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. ThinkingMach would still need to answer:
...
- real integration work for Codex/Cursor/Gemini-style ThinkingMach adapters

So the main near-term value is not universal replacement. It is selective use where compatibility is strong.

## Concrete recommendations for ThinkingMach
...
- keep ThinkingMach heartbeat/task/workspace/budget logic unchanged
...
- ThinkingMach's Codex adapter is already important and carries repo-specific behavior
...
- ThinkingMach still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, ThinkingMach should consider standardizing adapter/runtime permissions around a vocabulary like:
...
ThinkingMach should evaluate whether some execution workspaces can be backed by:
...
ThinkingMach plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into ThinkingMach core

ThinkingMach should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving ThinkingMach orchestration into agent-os workflows
...
`agent-os` is useful to ThinkingMach as an execution technology reference, not as a product model.

ThinkingMach should treat it the same way it treats sandboxes or agent CLIs:
