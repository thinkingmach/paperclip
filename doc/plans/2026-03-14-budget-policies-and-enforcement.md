...
ThinkingMach already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model ThinkingMach should implement next.

## Product Goals

ThinkingMach should let operators:
...
For the next implementation pass, ThinkingMach should enforce these defaults:
...
ThinkingMach should separate subscription-included usage from billed spend:
...
ThinkingMach should have two threshold classes:
...
ThinkingMach must know that a project is budget-paused and enforce it.
...
ThinkingMach must also block execution before new work starts.
