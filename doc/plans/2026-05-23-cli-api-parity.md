...
The most important requirement is credential ergonomics. External integrations need a reliable "way in" to ThinkingMach:
...
- `agent local-cli` creates an agent API key through board access, installs local skills, and prints `THINKINGMACH_API_URL`, `THINKINGMACH_COMPANY_ID`, `THINKINGMACH_AGENT_ID`, and `THINKINGMACH_API_KEY`.
...
The CLI must always know which ThinkingMach API it is operating against. This is especially important for fork/local development, where ThinkingMach may run on `3101+` rather than the upstream default `3100`.
...
- `thinkingmach connect` must show the resolved API base before any auth or mutation and allow the user to override it.
...
```json
{
  ...
  "profiles": {
    "default": {
      ...
      "persona": "agent",
      "agentId": "agent-id",
      "apiKeyEnvVarName": "THINKINGMACH_API_KEY"
    }
  }
}
```
...
```sh
thinkingmach token board create --company-id <company-id> --name "external-admin"
...
thinkingmach token agent revoke --agent <agent-id-or-name> <key-id>
```
...
```sh
thinkingmach agent-prompt <agent-name-or-id> <agent-api-key> "Prompt here"
```

Recommended safer variants:

```sh
thinkingmach agent prompt --agent <agent-name-or-id> --api-key-env THINKINGMACH_API_KEY "Prompt here"
thinkingmach agent prompt --profile my-agent "Prompt here"
thinkingmach board prompt --agent <agent-name-or-id> "Prompt here"
```
...
- Default prompt target should be `issue create + assign + wake`, because ThinkingMach's communication model is tasks/comments, not chat.
...
```sh
thinkingmach project list|get|create|update|delete
thinkingmach goal list|get|create|update|delete
```
...
```sh
thinkingmach issue child create <issue-id>
thinkingmach issue document list|get|put|delete|lock|unlock|revisions|restore
thinkingmach issue work-product list|create|update|delete
thinkingmach issue interaction list|create|accept|reject|respond|cancel
thinkingmach issue attachment list|upload|download|delete
thinkingmach issue force-release <issue-id>
thinkingmach issue label list|create|delete
thinkingmach issue read|unread|archive|unarchive
```
...
```sh
thinkingmach agent create|update|pause|resume|approve|terminate|delete
thinkingmach agent org
thinkingmach agent config get|revisions|rollback
thinkingmach agent instructions get|set|file
thinkingmach adapter list|models|profiles|detect|test|install|enable|disable|reload
```
...
```sh
thinkingmach cost summary|by-agent|by-project|by-provider|issue
thinkingmach cost event create
thinkingmach finance event create|list|summary
thinkingmach budget overview|set-company|set-agent|policy-create|incident-resolve
```
...
```sh
thinkingmach invite create|list|revoke|show|onboarding
thinkingmach join list|approve|reject|claim-key
thinkingmach member list|update|archive|permissions
thinkingmach admin user list|promote|demote|company-access
```
...
```sh
thinkingmach routine list|create|get|update|run|runs|trigger|revision
thinkingmach environment list|create|get|update|delete|probe|leases
thinkingmach workspace list|get|update|operations|runtime
thinkingmach project workspace list|create|update|delete|runtime
```
...
```text
thinkingmach connect
thinkingmach token board|agent create|list|revoke
thinkingmach whoami
thinkingmach prompt ...
thinkingmach board ...
thinkingmach agent ...
thinkingmach issue ...
thinkingmach project ...
thinkingmach goal ...
thinkingmach run ...
thinkingmach cost ...
thinkingmach budget ...
thinkingmach routine ...
thinkingmach environment ...
thinkingmach workspace ...
thinkingmach invite ...
thinkingmach member ...
thinkingmach plugin ...
thinkingmach instance ...
```

Alias policy:

- Keep existing commands working.
- Add aliases only for high-frequency flows, for example `thinkingmach ask` as an alias for `thinkingmach prompt`.
...
- Destructive tests must require an explicit test opt-in such as an env var or a dedicated test command; normal unit tests must not mutate a real running ThinkingMach instance.
...
- A new user can run `thinkingmach connect`, confirm or override the API base, select board or agent, and get a saved working profile tied to that API base.
...
```sh
thinkingmach agent-prompt AgentName "$AGENT_API_KEY" "Prompt here"
```

- The one-liner creates or updates ThinkingMach work, does not require a browser, and fails with a clear company/agent mismatch error when the token does not belong to the requested agent.
