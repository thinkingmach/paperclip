...
Yes, there is a likely root cause in ThinkingMach's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, ThinkingMach can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: ThinkingMach already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- ThinkingMach may think it cancelled the run because the parent process is gone
...
This work is successful when ThinkingMach can:
...
- require every skill author to add bespoke browser teardown logic before ThinkingMach can clean up correctly
...
- make the leak measurable from ThinkingMach's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by ThinkingMach
...
Treat this as a ThinkingMach executor ownership bug, not an `agent-browser` bug.
