...
Full ThinkingMach CLI/API parity smoke pass against a disposable local source-tree instance.

## Isolation Contract

- Repo: `/Users/aronprins/Documents/ThinkingMach/paperclip`
- Scratch root: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity`
- `THINKINGMACH_HOME`: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home`
- `THINKINGMACH_INSTANCE_ID`: `cli-api-parity`
- `THINKINGMACH_CONFIG`: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
- `THINKINGMACH_CONTEXT`: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/context.json`
- `THINKINGMACH_AUTH_STORE`: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/auth.json`
- `THINKINGMACH_API_URL`: `http://127.0.0.1:3197`
- `THINKINGMACH_SERVER_PORT`: `3197`
- `PORT`: `3197`
- `CODEX_HOME`: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/codex-home`
- `CLAUDE_HOME`: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/claude-home`
- `DATABASE_URL`: unset
...
- Actual result: Runbook requires explicit scratch paths, port `3197`, unset database env vars, `pnpm thinkingmach onboard --yes --run --bind loopback`, and pre-test isolation checks.
- Status: PASS.
...
- Prerequisites/IDs used: none.
- Expected result: All ThinkingMach/Codex/Claude paths point under `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` are unset; port `3197` has no listener.
...
- Follow-up: Start ThinkingMach with the runbook command.

### 2026-05-24T11:06:45+02:00 - Start isolated instance

- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm thinkingmach onboard --yes --run --bind loopback`
- Purpose: Create and start the disposable source-tree ThinkingMach instance.
...
- Actual result: Server started in `local_trusted/private` mode on `127.0.0.1:3197`, with embedded PostgreSQL data at `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/db`.
- Status: PASS.
...
- Command: `pnpm thinkingmach env`; `pnpm thinkingmach context show --json`; `curl -sS http://127.0.0.1:3197/api/health`; `find tmp/cli-api-parity/home -maxdepth 4 -type d`
...
- Actual result: Config path, context path, storage path, secrets key path, and DB directory all resolve under `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity`; health returned `status: ok`, version `0.3.3`, `deploymentMode: local_trusted`, `companyDeletionEnabled: true`.
- Status: PASS.
- Output summary: `thinkingmach env` redacted here because it prints the generated `THINKINGMACH_AGENT_JWT_SECRET`. Context existed at the scratch path with an empty `default` profile.
- Follow-up: Set API context and begin CLI parity checks.

### 2026-05-24T11:08:20+02:00 - Basic context/auth/connectivity

- Command: `pnpm thinkingmach context set --api-base http://127.0.0.1:3197 --use --json`; `pnpm thinkingmach whoami --json`; `pnpm thinkingmach company list --json`; `pnpm thinkingmach access whoami --json`
...
- Command: `pnpm thinkingmach company create --payload-json '{"name":"CLI API Parity Test","description":"Disposable company for CLI API parity testing","goal":"Exercise the CLI API surface end to end"}' --json`; `pnpm thinkingmach context set --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm thinkingmach company get 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm thinkingmach company update 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"description":"Updated by CLI API parity test","budgetMonthlyCents":12345}' --json`
...
- Command: edited `cli/src/commands/client/context.ts`, `cli/src/client/context.ts`, and `cli/src/__tests__/context.test.ts`; `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach context set --api-base http://127.0.0.1:3197 --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm thinkingmach context show --json`
...
- Output summary: The assigned `process` adapter agent ran automatically and generated heartbeat runs. The issue later moved to `blocked` via recovery handling because the smoke process printed output without a concrete ThinkingMach disposition.
...
- Command: `token board create --company-id ... --name cli-parity-board --never-expires --json`; `token board list --json`; `whoami --api-key <board-token> --json`; `token agent create --company-id ... --agent ... --name cli-parity-agent --json`; `token agent list --company-id ... --agent ... --json`; `context set --profile cli-agent --persona agent ... --api-key-env-var-name THINKINGMACH_API_KEY --json`; `agent me --profile cli-agent --json`; `agent inbox --profile cli-agent --json`; `issue list --profile cli-agent --company-id ... --json`; `company list --profile cli-agent --json`; `token agent revoke ...`; `token board revoke ...`
...
- Command: `kill <paperclip pid on 3197>`; `pnpm thinkingmach onboard --yes --run --bind loopback`; `curl http://127.0.0.1:3197/api/health`; `issue tree-hold:get <issue-id> null --json`
...
- Command: `curl -sf http://127.0.0.1:3197/api/health`; `pnpm thinkingmach health --json`; `pnpm thinkingmach adapter list/get/config-schema/ui-parser/models/model-profiles/detect-model/test-environment ... --json`; `pnpm thinkingmach environment list/capabilities/probe-config/create ... --json`; `pnpm thinkingmach project-workspace create/list/update/delete ... --json`; `pnpm thinkingmach workspace list --company-id ... --json`; `pnpm thinkingmach plugin list/examples/ui-contributions/tools/init ... --json`
...
- Command: `pnpm thinkingmach asset image:upload --company-id <company-id> --file doc/assets/avatars/zinc.png --namespace cli-parity --alt ... --title ... --json`; `pnpm thinkingmach asset content <asset-id> --out tmp/cli-api-parity/artifacts/asset-download.png --json`; `pnpm thinkingmach asset logo:upload --company-id <company-id> --file ui/public/favicon-32x32.png --json`; `pnpm thinkingmach skill list/create/get/file/file:update/update-status/install-update/delete/scan-projects ... --json`
...
- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm thinkingmach environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/ThinkingMach/paperclip"}}' --json`
- Purpose: Verify `BUG-004` against the restarted isolated source-tree server.
- Prerequisites/IDs used: Same scratch env; server restarted with `pnpm thinkingmach onboard --yes --run --bind loopback`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Command: `pnpm thinkingmach environment create/get/leases/probe/update/delete ... --json`; `pnpm thinkingmach plugin install/list/inspect/health/config/jobs/local-folders/ui-contributions/disable/enable/uninstall ... --json`; `pnpm thinkingmach secrets list/create/link/declarations/migrate-inline-env ... --json`
- Purpose: Add positive non-local environment coverage, plugin lifecycle coverage, and deeper secrets coverage.
- Prerequisites/IDs used: Company `12e9db4b-f66c-459b-959e-d645002240fb`; bundled plugin path `/Users/aronprins/Documents/ThinkingMach/paperclip/packages/plugins/plugin-workspace-diff`; temporary SSH environment `cc5ae311-13f5-42b8-8044-11065b4e1af0`; temporary plugin install `e8421ed5-c103-4950-afb7-1463a0fbb9c5`; temporary secret `20c74546-7bec-4766-80cd-0b6c57545f7d`.
...
- Command: `pnpm thinkingmach secrets update 20c74546-7bec-4766-80cd-0b6c57545f7d --payload-json ... --json`; `pnpm thinkingmach secrets rotate 20c74546-7bec-4766-80cd-0b6c57545f7d --value ... --json`; `pnpm thinkingmach secrets usage 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm thinkingmach secrets access-events 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm thinkingmach secrets delete 20c74546-7bec-4766-80cd-0b6c57545f7d --yes --confirm 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm thinkingmach secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`
...
- Expected result: `thinkingmach health` exists; `thinkingmach access whoami` works; `invite test-resolution` has a URL option; `join list --status pending` maps to `pending_approval`; issue help text no longer overstates valid cancel/recovery inputs.
...
- Command: `pnpm thinkingmach health --json`; `pnpm thinkingmach access whoami --json`; `pnpm thinkingmach join list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --status pending --request-type agent --json`; `pnpm thinkingmach invite create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{}' --json`; `pnpm thinkingmach invite test-resolution <token> --url https://example.com/invite/<token> --json`; `pnpm thinkingmach invite revoke <invite-id> --json`; `pnpm thinkingmach issue recovery:resolve --help`; `pnpm thinkingmach issue interaction:cancel --help`
...
- Prerequisites/IDs used: Isolated `CLAUDE_HOME=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/claude-home`.
...
- Status: PASS.
- Output summary: Mounted `llmRoutes` under `/api` in addition to the existing root mount. Updated available-skill discovery to read `CLAUDE_HOME/skills` when configured, include built-in ThinkingMach repo skills in `available-skill list`, and allow `available-skill get` for safe listed/built-in skill names.
...
- Command: `pnpm thinkingmach llm agent-configuration --json`; `pnpm thinkingmach llm agent-icons --json`; `pnpm thinkingmach llm agent-configuration:adapter process --json`; `pnpm thinkingmach available-skill list --json`; `pnpm thinkingmach available-skill get paperclip --json`; `pnpm thinkingmach available-skill get cmux --json`; `pnpm thinkingmach openapi --json`
...
- Expected result: LLM docs commands pass; built-in ThinkingMach skills are listed and fetchable; real-user `~/.claude` skills are not listed; `openapi` still documents the unresolved gap if no route exists.
- Actual result: LLM docs commands passed. `available-skill list` returned ThinkingMach repo skills such as `diagnose-why-work-stopped`; `available-skill get paperclip` returned markdown. `available-skill get cmux` now returns 404 because `cmux` is no longer listed from the real Claude home. `openapi` still returns `404: API route not found`.
- Status: MIXED.
...
- Command: environment echo; `pnpm thinkingmach health --json`; `pnpm thinkingmach token board list --json`; `pnpm thinkingmach token board revoke <redacted-board-token-id> --json`; `pnpm thinkingmach token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`; `pnpm thinkingmach plugin list --json`; `pnpm thinkingmach secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm thinkingmach environment list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm thinkingmach project-workspace list d32032ce-d95e-4c4e-a942-dd98498025fb --json`; `pnpm thinkingmach openapi --json`
...
- Command: `pnpm thinkingmach doctor --config <scratch-config>`; `pnpm thinkingmach doctor --config <scratch-config> --repair --yes`; `pnpm thinkingmach env --config <scratch-config>`; `pnpm thinkingmach db:backup --config <scratch-config> --dir tmp/cli-api-parity/artifacts/root-setup/backups --retention-days 1 --filename-prefix cli-parity --json`; `pnpm thinkingmach allowed-hostname cli-parity.test --config <scratch-config>`; `pnpm thinkingmach auth bootstrap-ceo --config <scratch-config> --force --base-url http://127.0.0.1:3197`; `pnpm thinkingmach auth whoami --json`; `pnpm thinkingmach routines disable-all --config <scratch-config> --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm thinkingmach env-lab doctor --instance cli-api-parity --json`; `pnpm thinkingmach env-lab status --instance cli-api-parity --json`
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Command: `THINKINGMACH_WORKTREES_DIR=tmp/cli-api-parity/worktrees-home pnpm thinkingmach worktree:list --json`; `pnpm thinkingmach worktree env --config <scratch-config> --json`; `pnpm thinkingmach worktree:merge-history --from current --to current --company CLI --dry`; `pnpm thinkingmach cloud push --company 12e9db4b-f66c-459b-959e-d645002240fb --dry-run --json`
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; scratch worktree root `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/worktrees-home`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Actual result: `worktree:list` passed and showed the current repo branch `improvement/cli-api-parity` with no ThinkingMach worktree config. `worktree env --config <scratch-config> --json` passed and printed the scratch `THINKINGMACH_CONFIG` plus generated env values; the generated JWT secret is intentionally not copied into this log. `worktree:merge-history --from current --to current --company CLI --dry` failed as expected with `Source and target ThinkingMach configs are the same. Choose different --from/--to worktrees.` `cloud push --dry-run` failed as expected with `Cloud sync is disabled. Enable the cloud sync experimental setting before running thinkingmach cloud push.`
...
- Command: `HOME=tmp/cli-api-parity/shell-home THINKINGMACH_WORKTREES_DIR=tmp/cli-api-parity/worktree-instances pnpm thinkingmach worktree:make cli-parity-wt --home <scratch-worktree-home> --from-config <scratch-config> --server-port 3198 --db-port 54331 --seed-mode minimal`; `pkill` only for the runaway scratch install attempt; edited `cli/src/commands/worktree.ts` and `cli/src/__tests__/worktree.test.ts`; `pnpm exec vitest run cli/src/__tests__/worktree.test.ts`; `pnpm --dir cli typecheck`; `thinkingmach worktree:cleanup cli-parity-wt --home <scratch-worktree-home> --force`; reran `thinkingmach worktree:make ...`; `thinkingmach worktree:list --json`; `thinkingmach worktree env --config <scratch-worktree-config> --json`; `thinkingmach worktree:merge-history --from paperclip-cli-parity-wt --to current --company CLI --dry`
...
- Prerequisites/IDs used: Scratch `HOME` `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/shell-home`; scratch worktree instance home `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/worktree-instances`; source config `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; worktree branch/path `paperclip-cli-parity-wt`.
...
- Purpose: Exercise cloud connect/push without a real external ThinkingMach Cloud stack, finish run subcommands that were not covered by earlier run list/get/events/log checks, and verify `connect` behavior in the non-interactive test runner.
...
- Expected result: Cloud sync can be enabled only in the scratch instance; `cloud connect` stores a fake upstream connection under scratch `THINKINGMACH_HOME`; `cloud push --dry-run` exports local company data and posts a preview bundle to the fake upstream; run read/control commands return structured results or controlled 404 for a nonexistent workspace operation; `connect` refuses non-interactive execution with guidance.
...
- Expected result: ThinkingMach remains running on `127.0.0.1:3197`; fake cloud port `3199` is stopped; no active board or agent tokens from the test remain; plugin list is empty; disposable routine is archived; `openapi` remains the one documented unresolved API route gap.
...
- Expected result: `/api/openapi.json` and `thinkingmach openapi --json` return a valid OpenAPI 3.0 document with the reference route inventory, including representative CLI/API parity paths such as `/api/companies/{companyId}/agents` and `/api/agents/{id}/keys`.
- Actual result: Focused test and `server` typecheck passed. After restart, direct curl returned `{"openapi":"3.0.0","pathCount":247}`. The CLI command returned `openapi: "3.0.0"`, title `ThinkingMach API`, `247` paths, `/api/openapi.json` summary `Get the generated OpenAPI document`, and `/api/agents/{id}/keys` POST summary `Create an agent API key`.
...
- Command: Edited `cli/src/commands/client/agent.ts`; ran `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm --silent thinkingmach agent instructions-path:update --help`.
...
- Actual result: Final handoff stated that the full isolated CLI/API parity pass completed and was committed, the scratch ThinkingMach server was still running on `http://127.0.0.1:3197`, and goal usage was 2,625,855 tokens over about 2h 23m.
- Status: PASS.
- Output summary:
...
- Final paths:
  - Scratch home: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home`
  - Config: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
  - Context: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/context.json`
  - Auth store: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/auth.json`
  - Embedded DB: `/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/db`, active on pg port `54330`
...
```sh
export THINKINGMACH_HOME=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home
export THINKINGMACH_INSTANCE_ID=cli-api-parity
export THINKINGMACH_CONFIG=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json
export THINKINGMACH_CONTEXT=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/context.json
export THINKINGMACH_AUTH_STORE=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/auth.json
export THINKINGMACH_API_URL=http://127.0.0.1:3197
export THINKINGMACH_SERVER_PORT=3197
export PORT=3197
export CODEX_HOME=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/codex-home
export CLAUDE_HOME=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/claude-home
unset DATABASE_URL DATABASE_MIGRATION_URL
pnpm thinkingmach health --json
```
...
- Command: `pnpm add @asteasolutions/zod-to-openapi@7.3.4 --filter @thinkingmach/server`; replaced `server/src/routes/openapi.ts` inventory stub with the schema-backed generator from `doc/plans/2026-05-23-cli-api-parity-openapi-reference.ts`; added route wrapper exports; tightened `server/src/__tests__/openapi-routes.test.ts`; `pnpm exec vitest run server/src/__tests__/openapi-routes.test.ts`; `pnpm --dir server typecheck`; restarted isolated server with the scratch env; `pnpm thinkingmach openapi --json`.
...
- Prerequisites/IDs used: Isolated scratch server restarted on `127.0.0.1:3197`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` unset; `THINKINGMACH_HOME`, `THINKINGMACH_CONFIG`, `THINKINGMACH_CONTEXT`, `THINKINGMACH_AUTH_STORE`, `CODEX_HOME`, and `CLAUDE_HOME` all under `tmp/cli-api-parity`.
- Expected result: `/api/openapi.json` and `thinkingmach openapi --json` return OpenAPI 3.0 with schema-backed request bodies, security schemes, public-operation security overrides, and create-operation `201` responses.
...
- Command: `pnpm thinkingmach health --json`; `pnpm thinkingmach openapi --json`; `lsof -nP -iTCP:3197 -sTCP:LISTEN`; `git status --short --branch`, all with the scratch `THINKINGMACH_*`, `CODEX_HOME`, and `CLAUDE_HOME` environment and with `DATABASE_URL`/`DATABASE_MIGRATION_URL` unset.
...
- Actual result: Health returned `status:"ok"`, version `0.3.3`, deployment mode `local_trusted`, exposure `private`, auth ready, bootstrap ready. OpenAPI returned `{openapi:"3.0.0", pathCount:259, security:["BoardSessionAuth","BoardApiKeyAuth","AgentBearerAuth"], companyCreateStatus:["201","400","401","403"]}`. `node` PID `84908` is listening on `127.0.0.1:3197`. Git status was clean before this final log update.
- Status: PASS.
...
- Command: Stopped the foreground scratch server process; started the same runbook command in detached screen session `paperclip-cli-parity`; verified `pnpm thinkingmach health --json`; checked `lsof -nP -iTCP:3197 -sTCP:LISTEN`; checked `screen -ls`.
...
- Reproduction command: Install `packages/plugins/examples/plugin-kitchen-sink-example`, then run `pnpm thinkingmach plugin tool:execute --payload-json '{"tool":"paperclip-kitchen-sink-example:echo","parameters":{"message":"CLI parity tool"},"runContext":{"companyId":"<company-id>","projectId":"<project-id>","agentId":"<agent-id>","runId":"<run-id>"}}' --json`.
...
- Reproduction command: `pnpm thinkingmach token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`.
...
- Reproduction command: `HOME=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/shell-home pnpm thinkingmach worktree:make cli-parity-wt --home /Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/worktree-instances --from-config /Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json --server-port 3198 --db-port 54331 --seed-mode minimal`.
...
- Suspected cause: The CLI did not reuse the pnpm executable that launched the current ThinkingMach command, so dependency installation was subject to PATH/shim behavior under an overridden `HOME`.
...
- Remaining risk: Low. If ThinkingMach is launched outside pnpm, dependency installation still falls back to PATH lookup as before.

### BUG-008 - `configure --section <invalid>` printed an error but exited 0

- Status: Fixed and live-verified.
- Severity: Low command UX/scripting bug.
- Reproduction command: `pnpm thinkingmach configure --config /Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json --section invalid-section`.
...
- Status: Fixed.
- Severity: High for isolated CLI testing; a non-default `apiBase` can be silently removed and later commands may fall back to `http://localhost:3100` if `THINKINGMACH_API_URL` is absent.
- Reproduction command: `pnpm thinkingmach context set --api-base http://127.0.0.1:3197 --use --json`; then `pnpm thinkingmach context set --company-id <company-id> --use --json`; then `pnpm thinkingmach context show --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach context show --json`.
...
- Reproduction command: `pnpm thinkingmach access whoami --json`.
...
- Fix summary: Added `thinkingmach access whoami` as an alias for the existing top-level `whoami` command.
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach access whoami --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm thinkingmach issue interaction:accept <issue-id> <request-confirmation-interaction-id> --json`.
...
- Reproduction command: `pnpm thinkingmach issue tree-hold:get <issue-id> null --json` or `pnpm thinkingmach issue tree-hold:release <issue-id> null --json`.
...
- Reproduction command: `pnpm thinkingmach issue interaction:cancel <issue-id> <request-confirmation-interaction-id> --reason "..." --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach issue interaction:cancel --help`.
...
- Status: Fixed help text.
- Severity: Low command UX drift.
- Reproduction command: `pnpm thinkingmach issue recovery:resolve <issue-id> --action-id <action-id> --outcome restored --source-issue-status blocked --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach issue recovery:resolve --help`.
...
- Severity: Low command UX drift.
- Reproduction command: `pnpm thinkingmach agent instructions-path:update <process-agent-id> --payload-json '{"path":"docs/cli-parity.md"}' --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach agent instructions-path:update --help`.
...
- Reproduction command: `pnpm thinkingmach invite test-resolution <invite-token> --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach invite test-resolution <token> --url https://example.com/invite/<token> --json`.
...
- Status: Fixed and live-verified.
- Severity: Low command UX drift.
- Reproduction command: `pnpm thinkingmach join list --company-id <company-id> --status pending --request-type agent --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach join list --company-id <company-id> --status pending --request-type agent --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm thinkingmach openapi --json`; `pnpm thinkingmach available-skill get cmux --json`; `pnpm thinkingmach llm agent-configuration --json`; `pnpm thinkingmach llm agent-icons --json`; `pnpm thinkingmach llm agent-configuration:adapter process --json`.
...
- Fix summary: Mounted LLM docs routes under `/api`; made available-skill discovery honor `CLAUDE_HOME`, include built-in ThinkingMach repo skills, and fetch safe skill markdown consistently; added `/api/openapi.json`, then upgraded it from the initial path inventory to the schema-backed `OpenAPIRegistry`/`OpenApiGeneratorV3` implementation from the parity reference.
- Verification command: `pnpm exec vitest run server/src/__tests__/llms-routes.test.ts cli/src/__tests__/access-parity.test.ts`; `pnpm --dir server typecheck`; `pnpm --dir cli typecheck`; live `llm` and `available-skill` commands after restart; `pnpm exec vitest run server/src/__tests__/openapi-routes.test.ts`; live `curl http://127.0.0.1:3197/api/openapi.json`; live `pnpm thinkingmach openapi --json`; follow-up live schema-backed `thinkingmach openapi --json`.
...
- Reproduction command: `CLAUDE_HOME=/Users/aronprins/Documents/ThinkingMach/paperclip/tmp/cli-api-parity/claude-home pnpm thinkingmach available-skill list --json`.
- Expected result: Skill discovery uses the isolated Claude home or built-in repo skills only.
- Actual result: Before the fix, the list included `cmux` from the real user Claude skills home, and `available-skill get cmux` failed because only a hardcoded ThinkingMach subset was fetchable.
- Suspected cause: Server code read `HOME/.claude/skills` directly and did not add built-in ThinkingMach skills unless they were present in Claude's skills directory.
- Files changed: `server/src/routes/access.ts`, `doc/bugs/2026-05-24-cli-api-parity-e2e-log.md`.
- Fix summary: Use `CLAUDE_HOME/skills` when `CLAUDE_HOME` is set, include built-in ThinkingMach skills in catalog output, and resolve safe skill markdown from both Claude and ThinkingMach skills directories.
...
### MISMATCH-008 - `thinkingmach health` is not registered

- Status: Fixed and live-verified.
- Severity: Low command/API parity gap.
- Reproduction command: `pnpm thinkingmach health --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm thinkingmach health --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm thinkingmach environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/ThinkingMach/paperclip"}}' --json`.
...
- Status: Fixed and live-verified.
- Severity: Medium CLI/API parity gap.
- Reproduction command: `pnpm thinkingmach secrets --help` did not expose commands for `PATCH /api/secrets/:id`, `POST /api/secrets/:id/rotate`, `GET /api/secrets/:id/usage`, `GET /api/secrets/:id/access-events`, or `DELETE /api/secrets/:id`.
