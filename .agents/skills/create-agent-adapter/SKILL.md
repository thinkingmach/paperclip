...
description: >
  Create or modify ThinkingMach agent adapters across server, UI, and CLI surfaces.
  Use when adding support for a new CLI agent, API agent, custom process, or
  adapter package.
---

# Creating a ThinkingMach Agent Adapter

An adapter bridges ThinkingMach's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `thinkingmach run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@thinkingmach/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@thinkingmach/adapter-utils` (types) or `@thinkingmach/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell ThinkingMach to forget the stored session
}
...
```json
{
  "name": "@thinkingmach/adapter-<name>",
  "version": "0.0.1",
  "private": true,
  "type": "module",
  "exports": {
  ...
  "dependencies": {
    "@thinkingmach/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including ThinkingMach agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@thinkingmach/adapter-utils/server-utils`)
2. **Build environment** — call `buildThinkingMachEnv(agent)` then layer in `THINKINGMACH_RUN_ID`, context vars (`THINKINGMACH_TASK_ID`, `THINKINGMACH_WAKE_REASON`, `THINKINGMACH_WAKE_COMMENT_ID`, `THINKINGMACH_APPROVAL_ID`, `THINKINGMACH_APPROVAL_STATUS`, `THINKINGMACH_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `THINKINGMACH_API_URL` | Server's own URL |
| `THINKINGMACH_RUN_ID` | Current run id |
| `THINKINGMACH_TASK_ID` | `context.taskId` or `context.issueId` |
| `THINKINGMACH_WAKE_REASON` | `context.wakeReason` |
| `THINKINGMACH_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `THINKINGMACH_APPROVAL_ID` | `context.approvalId` |
| `THINKINGMACH_APPROVAL_STATUS` | `context.approvalStatus` |
| `THINKINGMACH_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `THINKINGMACH_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `thinkingmach run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@thinkingmach/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@thinkingmach/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use ThinkingMach API
...
import { parseMyAgentStdoutLine } from "@thinkingmach/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@thinkingmach/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@thinkingmach/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so ThinkingMach wipes the stale session
...
Import from `@thinkingmach/adapter-utils/server-utils`:
...
| `buildThinkingMachEnv(agent)` | Standard `THINKINGMACH_*` env vars |
...
- Package name: `@thinkingmach/adapter-<kebab-name>`
...
- Default prompt should use `DEFAULT_THINKINGMACH_AGENT_PROMPT_TEMPLATE` from `@thinkingmach/adapter-utils/server-utils` so local adapters share ThinkingMach's execution contract: act in the same heartbeat, avoid planning-only exits unless requested, leave durable progress and a next action, use child issues instead of polling, mark blockers with owner/action, and respect governance boundaries.
...
### ThinkingMach Skills Injection

ThinkingMach ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `paperclip` API skill and the `paperclip-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with ThinkingMach internals, break git status, and potentially leak into commits.
...
async function buildSkillsDir(): Promise<string> {
  ...
  const entries = await fs.readdir(THINKINGMACH_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(THINKINGMACH_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks ThinkingMach skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
- `THINKINGMACH_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the ThinkingMach API and GitHub should not have access to arbitrary hosts.
