...
import { resolveThinkingMachConfigPath, resolveThinkingMachEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@thinkingmach/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.THINKINGMACH_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.THINKINGMACH_AGENT_JWT_SECRET === "string" ? parsed.THINKINGMACH_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `npx thinkingmach onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveThinkingMachEnvPath();
