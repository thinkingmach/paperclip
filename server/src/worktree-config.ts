...
function emptyWorktreeEnvFileContents(): string {
  return [
    ...
    "# Generated by ThinkingMach worktree repair",
    "",
  ].join("\n");
}
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.THINKINGMACH_IN_WORKTREE !== "true") return null;

  const configPath = resolveThinkingMachConfigPath(overrideConfigPath);
  const envPath = resolveThinkingMachEnvPath(configPath);
  const persistedEnv = readEnvEntries(envPath);

  // THINKINGMACH_IN_WORKTREE can leak in from a parent process or a sourced env
  ...
  if (persistedEnv.THINKINGMACH_IN_WORKTREE !== "true") return null;

  const persistedConfigPath = nonEmpty(persistedEnv.THINKINGMACH_CONFIG);
  ...
  const worktreeName =
    nonEmpty(stablePersistedEnv.THINKINGMACH_WORKTREE_NAME) ??
    nonEmpty(env.THINKINGMACH_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(stablePersistedEnv.THINKINGMACH_INSTANCE_ID) ??
    nonEmpty(env.THINKINGMACH_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(stablePersistedEnv.THINKINGMACH_HOME) ??
      nonEmpty(env.THINKINGMACH_HOME) ??
      nonEmpty(env.THINKINGMACH_WORKTREES_DIR) ??
      "~/.paperclip-worktrees",
  );
...
function writeConfigFile(configPath: string, config: ThinkingMachConfig): boolean {
  ...
  const nextConfig = source
    ? paperclipConfigSchema.parse(mergeThinkingMachConfig(source, update))
    : update;
...
function collectSiblingWorktreePorts(
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as ThinkingMachConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: ThinkingMachConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: ThinkingMachConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.THINKINGMACH_HOME = context.homeDir;
  process.env.THINKINGMACH_INSTANCE_ID = context.instanceId;
  process.env.THINKINGMACH_CONFIG = context.configPath;
  process.env.THINKINGMACH_CONTEXT = context.contextPath;
  process.env.THINKINGMACH_WORKTREE_NAME = context.worktreeName;
  ...
  if (fs.existsSync(context.configPath)) {
    try {
      const runtimeConfig = withWorktreePortRegistryLockSync(context.homeDir, () => {
        const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ThinkingMachConfig;
  ...
  const toolActionSigningSecret =
    ...
    nonEmpty(existingEnvEntries.THINKINGMACH_TOOL_ACTION_SIGNING_SECRET) ??
    randomBytes(32).toString("hex");

  const managedEnvEntries: Record<string, string> = {
    THINKINGMACH_HOME: context.homeDir,
    THINKINGMACH_INSTANCE_ID: context.instanceId,
    THINKINGMACH_CONFIG: context.configPath,
    THINKINGMACH_CONTEXT: context.contextPath,
    THINKINGMACH_IN_WORKTREE: "true",
    THINKINGMACH_DB_BACKUP_ENABLED: "false",
    THINKINGMACH_WORKTREE_NAME: context.worktreeName,
    THINKINGMACH_TOOL_ACTION_SIGNING_SECRET: toolActionSigningSecret,
  };

  process.env.THINKINGMACH_DB_BACKUP_ENABLED = "false";
  process.env.THINKINGMACH_TOOL_ACTION_SIGNING_SECRET = toolActionSigningSecret;
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: ThinkingMachConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ThinkingMachConfig;
  } catch {
    return;
  }
