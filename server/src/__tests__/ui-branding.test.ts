...
<head>
    <!-- THINKINGMACH_RUNTIME_BRANDING_START -->
    <!-- THINKINGMACH_RUNTIME_BRANDING_END -->
    <!-- THINKINGMACH_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- THINKINGMACH_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from THINKINGMACH_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ THINKINGMACH_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ THINKINGMACH_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ THINKINGMACH_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      THINKINGMACH_IN_WORKTREE: "true",
      THINKINGMACH_WORKTREE_NAME: "paperclip-pr-432",
      THINKINGMACH_WORKTREE_COLOR: "#4f86f7",
    });
  ...
  it("surfaces the runtime instance id so the UI can fail closed on copied rows", () => {
    const branding = getWorktreeUiBranding({
      ...
      THINKINGMACH_WORKTREE_NAME: "paperclip-pr-432",
      THINKINGMACH_WORKTREE_COLOR: "#4f86f7",
      THINKINGMACH_INSTANCE_ID: "inst-abc123",
    });
