...
describe("agent instructions service", () => {
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const thinkingmachHome = await makeTempDir("paperclip-agent-instructions-home-");
    const externalRoot = await makeTempDir("paperclip-agent-instructions-external-");
    cleanupDirs.add(thinkingmachHome);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const thinkingmachHome = await makeTempDir("paperclip-agent-instructions-recover-");
    cleanupDirs.add(thinkingmachHome);
    process.env.THINKINGMACH_HOME = thinkingmachHome;
    process.env.THINKINGMACH_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const thinkingmachHome = await makeTempDir("paperclip-agent-instructions-stale-managed-");
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const thinkingmachHome = await makeTempDir("paperclip-agent-instructions-heal-write-");
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const thinkingmachHome = await makeTempDir("paperclip-agent-instructions-heal-delete-");
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const thinkingmachHome = await makeTempDir("paperclip-agent-instructions-partial-managed-");
