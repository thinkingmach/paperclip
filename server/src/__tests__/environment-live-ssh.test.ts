...
function resolveEnvLabStatePath(): string {
  const instanceRoot =
    process.env.THINKINGMACH_INSTANCE_ROOT?.trim() ||
...
/** Attempt to build config from explicit THINKINGMACH_ENV_LIVE_SSH_* env vars. */
function tryExplicitConfig(): {
  ...
  const host = process.env.THINKINGMACH_ENV_LIVE_SSH_HOST?.trim() ?? "";
  const username = process.env.THINKINGMACH_ENV_LIVE_SSH_USERNAME?.trim() ?? "";
  const remoteWorkspacePath =
    process.env.THINKINGMACH_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH?.trim() ?? "";
  const port = Number.parseInt(process.env.THINKINGMACH_ENV_LIVE_SSH_PORT ?? "22", 10);
 ...
 * 1. Explicit THINKINGMACH_ENV_LIVE_SSH_* env vars
...
async function resolveSshConfig(): Promise<SshConnectionConfig | null> {
  ...
  if (explicit) {
    return {
      ...
      privateKey: await readOptionalSecret(
        process.env.THINKINGMACH_ENV_LIVE_SSH_PRIVATE_KEY,
        process.env.THINKINGMACH_ENV_LIVE_SSH_PRIVATE_KEY_PATH,
      ),
      knownHosts: await readOptionalSecret(
        process.env.THINKINGMACH_ENV_LIVE_SSH_KNOWN_HOSTS,
        process.env.THINKINGMACH_ENV_LIVE_SSH_KNOWN_HOSTS_PATH,
      ),
      strictHostKeyChecking:
        (process.env.THINKINGMACH_ENV_LIVE_SSH_STRICT_HOST_KEY_CHECKING ?? "true").toLowerCase() !== "false",
    };
  }
  ...
  if (process.env.THINKINGMACH_ENV_LIVE_SSH_NO_AUTO_FIXTURE !== "true") {
...
const describeLiveSsh = (() => {
  ...
  if (process.env.THINKINGMACH_ENV_LIVE_SSH_NO_AUTO_FIXTURE === "true") {
    console.warn(
      "Skipping live SSH smoke test. Set THINKINGMACH_ENV_LIVE_SSH_HOST, THINKINGMACH_ENV_LIVE_SSH_USERNAME, and THINKINGMACH_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH to enable it, or remove THINKINGMACH_ENV_LIVE_SSH_NO_AUTO_FIXTURE to auto-start env-lab.",
    );
...
describeLiveSsh("live SSH environment smoke", () => {
  ...
  it("connects to the configured SSH environment and verifies basic runtime tools", async () => {
    ...
    if (!resolvedConfig) {
      console.warn(
        "Live SSH smoke test could not resolve SSH config from env vars or env-lab fixture. Set THINKINGMACH_ENV_LIVE_SSH_NO_AUTO_FIXTURE=true to mark this suite skipped intentionally.",
      );
      return;
    }
