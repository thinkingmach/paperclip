...
describeEmbeddedPostgres("company skill import authorization routes", () => {
  ...
  beforeAll(async () => {
    process.env.THINKINGMACH_AGENT_JWT_SECRET = "company-skills-import-authz-test-secret";
    thinkingmachHome = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-company-skills-import-authz-home-"));
  ...
  afterAll(async () => {
    ...
    if (previousAgentJwtSecret === undefined) delete process.env.THINKINGMACH_AGENT_JWT_SECRET;
    else process.env.THINKINGMACH_AGENT_JWT_SECRET = previousAgentJwtSecret;
