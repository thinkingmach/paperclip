...
import { testEnvironment } from "@thinkingmach/adapter-gemini-local/server";
...
const outPath = process.env.THINKINGMACH_TEST_ARGS_PATH;
if (outPath) {
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          THINKINGMACH_TEST_ARGS_PATH: argsCapturePath,
