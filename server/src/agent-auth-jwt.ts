...
import { normalizeAgentApiKeyScope, type AgentApiKeyScope } from "@thinkingmach/shared";
...
function jwtConfig() {
  const secret = process.env.THINKINGMACH_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  ...
  return {
    ...
    ttlSeconds: parseNumber(process.env.THINKINGMACH_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.THINKINGMACH_AGENT_JWT_ISSUER ?? "paperclip",
    audience: process.env.THINKINGMACH_AGENT_JWT_AUDIENCE ?? "paperclip-api",
    ...
    // name) even though it deliberately shares THINKINGMACH_AGENT_JWT_SECRET with
    ...
    disableLegacyFallback: parseBooleanEnv(process.env.THINKINGMACH_AGENT_JWT_DISABLE_LEGACY_FALLBACK),
  };
}
 ...
 * have expired (set THINKINGMACH_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true). Normal
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  //
  // Operators should set `THINKINGMACH_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true`
  // approximately one JWT TTL (~48h by default, see THINKINGMACH_AGENT_JWT_TTL_SECONDS)
