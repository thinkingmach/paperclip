...
describe("parseAdapterRegistryEnv", () => {
  ...
  it("parses inline THINKINGMACH_ADAPTERS JSON", () => {
    const r = parseAdapterRegistryEnv({ THINKINGMACH_ADAPTERS: ENTRY });
  ...
  it("throws on malformed JSON (fail loud)", () => {
    expect(() => parseAdapterRegistryEnv({ THINKINGMACH_ADAPTERS: "{not json" })).toThrow(
      /THINKINGMACH_ADAPTERS/,
    );
  });

  it("throws on schema-invalid content (fail loud)", () => {
    expect(() =>
      parseAdapterRegistryEnv({ THINKINGMACH_ADAPTERS: JSON.stringify([{ enabled: true }]) }),
    ).toThrow(/THINKINGMACH_ADAPTERS/);
  });
});
