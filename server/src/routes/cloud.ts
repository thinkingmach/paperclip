...
export function cloudRoutes(opts: {
  ...
  router.get("/stacks", async (req, res) => {
    ...
    const tenantToken = runtimeEnv.THINKINGMACH_CLOUD_TENANT_SERVER_TOKEN?.trim();
    if (!context.cloudOrigin || !context.stackId || !tenantToken) {
      throw new HttpError(503, "ThinkingMach Cloud portfolio is not configured", {
    ...
    try {
      ...
      if (!upstream.ok) {
        logger.warn(
          ...
          "ThinkingMach Cloud portfolio request failed",
        );
        throw new HttpError(502, "ThinkingMach Cloud portfolio request failed", {
      ...
      try {
        ...
        throw new HttpError(502, "ThinkingMach Cloud portfolio returned invalid JSON", {
