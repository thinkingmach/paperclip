...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole } from "@thinkingmach/shared";
import {
  forbidden,
  conflict,
  notFound,
 ...
 * (`instance.access` in THINKINGMACH_HIDDEN_SETTINGS), instance-admin user
...
/** Resolve the ThinkingMach repo skills directory (built-in / managed skills). */
function resolveThinkingMachSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  const claudeSkillsDir = resolveClaudeSkillsDir();
  const paperclipSkillsDir = resolveThinkingMachSkillsDir();

  // Build set of ThinkingMach-managed skill names
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isThinkingMachManaged: paperclipSkillNames.has(entry.name),
      });
    }
    ...
    try {
      for (const entry of fs.readdirSync(paperclipSkillsDir, { withFileTypes: true })) {
        ...
        skills.push({
          name: entry.name,
          description,
          isThinkingMachManaged: true,
        });
      }
    } catch { /* skip ThinkingMach skills directory */ }
  }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.paperclipApiUrl)) {
    const legacyThinkingMachApiUrl = nonEmptyTrimmedString(input.paperclipApiUrl);
    if (legacyThinkingMachApiUrl) merged.paperclipApiUrl = legacyThinkingMachApiUrl;
  }
...
export function normalizeAgentDefaultsForJoin(input: {
        ...
        if (apiBaseUrl.protocol !== "http:" && apiBaseUrl.protocol !== "https:") {
          ...
          if (mappedDefaultDashboardRoot) {
            diagnostics.push({
              ...
              hint: "Hermes dashboard and /chat routes are browser UI routes. ThinkingMach gateway calls use /api/health and /api/v1/runs.",
            });
          }
  ...
  const rawThinkingMachApiUrl =
    typeof defaults.paperclipApiUrl === "string"
      ? defaults.paperclipApiUrl.trim()
      : "";
  if (rawThinkingMachApiUrl) {
    try {
      const parsedThinkingMachApiUrl = new URL(rawThinkingMachApiUrl);
      if (
        parsedThinkingMachApiUrl.protocol !== "http:" &&
        parsedThinkingMachApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_paperclip_api_url_protocol",
          level: "warn",
          message: `paperclipApiUrl must use http:// or https:// (got ${parsedThinkingMachApiUrl.protocol}).`
        });
      } else {
        normalized.paperclipApiUrl = parsedThinkingMachApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_paperclip_api_url_configured",
          level: "info",
          message: `paperclipApiUrl set to ${parsedThinkingMachApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        ...
        level: "warn",
        message: `Invalid paperclipApiUrl: ${rawThinkingMachApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your ThinkingMach host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "ThinkingMach is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run npx thinkingmach allowed-hostname <host>`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an external ThinkingMach agent, save your one-time claim secret, wait for board approval, then claim your ThinkingMach API key through the standard claim endpoint. Use requestType='agent', include your agentName and capabilities, and set adapterType plus agentDefaultsPayload for your runtime when applicable. Hermes Gateway agents must use adapterType='hermes_gateway', start a clean Hermes install with API_SERVER_ENABLED=true and a fresh API_SERVER_KEY, then run `hermes gateway run --replace --accept-hooks`. Put the Hermes gateway URL in agentDefaultsPayload.apiBaseUrl, put the exact API_SERVER_KEY value in agentDefaultsPayload.apiKey, and put the reachable ThinkingMach base URL in agentDefaultsPayload.paperclipApiUrl. If you use the default Hermes dashboard root or /chat URL on port 9119, ThinkingMach maps it to /api automatically. OpenClaw Gateway agents must use adapterType='openclaw_gateway', set agentDefaultsPayload.url to a ws:// or wss:// gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token.",
      inviteMessage: extractInviteMessage(invite),
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the ThinkingMach hostname is reachable and allowed via `npx thinkingmach allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this ThinkingMach API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific ThinkingMach skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Decide which ThinkingMach adapter type matches your runtime.

    Use adapterType only when there is a matching ThinkingMach adapter. Put runtime-specific settings in agentDefaultsPayload.
    ...
    - Set agentDefaultsPayload.apiBaseUrl to the Hermes gateway URL ThinkingMach can reach.
    - Set agentDefaultsPayload.apiKey to the exact same value as API_SERVER_KEY. This is the Hermes gateway key, not the ThinkingMach key.
    ...
    - Watch out: /chat and the dashboard root are browser UI routes. ThinkingMach tests /api/health and starts runs with /api/v1/runs after mapping them to the API base.
    - Set agentDefaultsPayload.paperclipApiUrl to the ThinkingMach base URL Hermes can reach.
    - Use hermes_local when ThinkingMach should start Hermes on the ThinkingMach host.
    - Use hermes_gateway when ThinkingMach should call an already-running Hermes API server.
    - After board approval, claim the ThinkingMach API key once with the claim endpoint below and save it as THINKINGMACH_API_KEY. Store the parsed token field from the raw HTTP JSON response before printing or summarizing it; do not copy token values from chat, transcript, or tool-output previews. A token value containing literal ... or [redacted] is a masked display preview, not a valid key. Do not rotate or invent a ThinkingMach key manually.
    - Hermes-originated ThinkingMach API usage means Hermes calls ThinkingMach with THINKINGMACH_API_URL and THINKINGMACH_API_KEY after approval/key claim. Do not confuse that with agentDefaultsPayload.apiBaseUrl, which points ThinkingMach to Hermes.
    ...
    - Local loopback dashboard root or chat URL: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:9119" or "http://127.0.0.1:9119/chat"; ThinkingMach maps either one to "http://127.0.0.1:9119/api".
    ...
    - Docker: if ThinkingMach runs in Docker and Hermes runs on the host, use agentDefaultsPayload.apiBaseUrl = "http://host.docker.internal:8642"; if both run in Compose, use the Hermes service name.
    ...
    The board approves the join request in ThinkingMach before key claim is allowed.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set THINKINGMACH_API_KEY and THINKINGMACH_API_URL for future ThinkingMach API calls. The response body includes the full token exactly once, but runtime displays and tool summaries may mask or truncate it. Write the raw response token directly to private storage before logging anything, then verify it with an authenticated ThinkingMach API call. Do not persist displayed previews containing literal ... or [redacted].
    ...
    ## Step 4: Install ThinkingMach skill
    ...
    ${
      ...
      "Ensure ThinkingMach is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested ThinkingMach base URLs to try");
    ...
    appendBlock(`
      ...
      - npx thinkingmach allowed-hostname <host>
      - then restart ThinkingMach and retry onboarding.
    `);
  }
