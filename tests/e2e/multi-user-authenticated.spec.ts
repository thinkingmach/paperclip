...
const BASE = process.env.THINKINGMACH_E2E_BASE_URL ?? "http://127.0.0.1:3105";
const DATA_DIR = process.env.THINKINGMACH_E2E_DATA_DIR ?? process.env.THINKINGMACH_HOME;
const CONFIG_PATH = process.env.THINKINGMACH_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".paperclip/config.json");
...
function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("THINKINGMACH_E2E_DATA_DIR or THINKINGMACH_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to ThinkingMach" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your ThinkingMach account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your ThinkingMach instance" })).toBeVisible();
