 ...
 *
 * Plugin bundles are built with `external: ["@thinkingmach/plugin-sdk/ui", "react", "react-dom"]`,
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        ...
        if (!R) {
          throw new Error("ThinkingMach plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
    ...
    case "sdk-ui":
      source = `
        ...
        function missing(name) {
          return function MissingThinkingMachSdkUiComponent() {
            throw new Error('ThinkingMach plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@thinkingmach/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@thinkingmach/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@thinkingmach/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@thinkingmach/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@thinkingmach/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
 ...
 * of `@thinkingmach/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
