...
import { redactCommandText as redactCommandSecretText } from "@thinkingmach/adapter-utils";
...
import { redactHomePathUserSegments, redactHomePathUserSegmentsInValue } from "@thinkingmach/adapter-utils";
...
const COMMAND_ENV_KEY_RE = /(^command$|^cmd$|command[-_]?line|resolved[-_]?command|THINKINGMACH_RESOLVED_COMMAND)/i;
  ...
  return (
    <div className={cn("space-y-6", isMobile && showConfigActionBar && "pb-24")}>
      ...
      {builtInState && (
        ...
        >
          Ships with ThinkingMach and powers <strong>{builtInFeatureLabel}</strong>. Configure it like
                ...
                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="right" sideOffset={4}>
                      Managed: ThinkingMach stores and serves the instructions bundle. External: you provide a path on disk where the instructions live.
                    </TooltipContent>
                  </Tooltip>
                </span>
              ...
              <label className="space-y-1.5 min-w-0">
                <span className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="right" sideOffset={4}>
                      The absolute directory on disk where the instructions bundle lives. In managed mode this is set by ThinkingMach automatically.
                    </TooltipContent>
                  </Tooltip>
                </span>
                {currentMode === "managed" ? (
...
function RunDetail({ run: initialRun, agentRouteId, adapterType, adapterConfig }: { run: HeartbeatRun; agentRouteId: string; adapterType: string; adapterConfig: Record<string, unknown> }) {
  ...
  const paperclipDeveloperMode =
    experimentalSettings?.enableThinkingMachDeveloperMode === true;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          API keys allow this agent to authenticate calls to the ThinkingMach server.
        </p>
        <div className="flex items-center gap-2">
          <Input
