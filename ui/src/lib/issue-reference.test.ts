...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal ThinkingMach issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/thinkingmach/paperclip/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/thinkingmach/paperclip/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute ThinkingMach issue URLs so origin, port, and hash are not lost", () => {
