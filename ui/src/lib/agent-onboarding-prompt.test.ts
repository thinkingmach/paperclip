...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the ThinkingMach API key");
    ...
    expect(prompt).toContain("ThinkingMach maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the ThinkingMach host");
    expect(prompt).toContain("Hermes-originated ThinkingMach API calls");
    expect(prompt).toContain("Do not rotate or invent a ThinkingMach key manually");
