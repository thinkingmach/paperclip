...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@thinkingmach/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@thinkingmach/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@thinkingmach/adapter-utils").TranscriptEntry[];
