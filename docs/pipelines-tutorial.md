...
Run this against a dev ThinkingMach instance with a board token or an agent token that can manage pipelines, routines, and issues.
...
thinkingmach pipelines create \
  -C "$THINKINGMACH_COMPANY_ID" \
...
thinkingmach pipelines set-transitions \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$RELEASE_PIPELINE" \
...
thinkingmach pipelines guidance put \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
...
export DRAFTING_ROUTINE_ID="$(
  thinkingmach routine create \
    -C "$THINKINGMACH_COMPANY_ID" \
    --payload-json "$(jq -c . /tmp/drafting-routine.json)" \
    --json | jq -r '.id'
)"

thinkingmach pipelines set-automation \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
  --stage drafting \
...
thinkingmach pipelines ingest-batch \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$FEATURE_PIPELINE" \
...
thinkingmach pipelines review-bulk \
  -C "$THINKINGMACH_COMPANY_ID" \
...
export SUGGESTION_ID="$(
  thinkingmach pipelines case suggest \
    -C "$THINKINGMACH_COMPANY_ID" \
    "$BLOG_CASE" \
    --to assets \
...
thinkingmach pipelines case resolve-suggestion \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$BLOG_CASE" \
...
curl -sS -X POST \
  ...
  "$THINKINGMACH_API_URL/api/cases/$TWEET_CASE/issue-links" >/dev/null

thinkingmach pipelines case edit \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$BLOG_CASE" \
  --expected-version 2 \
...
# Recovery:
thinkingmach pipelines case get -C "$THINKINGMACH_COMPANY_ID" "$BLOG_CASE" --json
```
...
```sh
export BLOG_VERSION="$(thinkingmach pipelines case get -C "$THINKINGMACH_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
thinkingmach pipelines case transition -C "$THINKINGMACH_COMPANY_ID" "$HERO_CASE" --to published --expected-version 1 --reason "Hero image done."
thinkingmach pipelines case transition -C "$THINKINGMACH_COMPANY_ID" "$CARD_CASE" --to dropped --expected-version 1 --reason "Social card not needed."
```

When both asset cases are terminal, move the blog case to `assembly`.

```sh
export BLOG_ASSETS_VERSION="$(thinkingmach pipelines case get -C "$THINKINGMACH_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

thinkingmach pipelines case transition \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$BLOG_CASE" \
  --to assembly \
...
thinkingmach pipelines case transition -C "$THINKINGMACH_COMPANY_ID" "$ASSEMBLY_CASE" --to published --expected-version 1 --reason "Assembly complete."
```
...
```sh
export BLOG_REVIEW_VERSION="$(thinkingmach pipelines case get -C "$THINKINGMACH_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

thinkingmach pipelines case review \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$BLOG_CASE" \
  --approve \
...
```sh
thinkingmach pipelines case transition -C "$THINKINGMACH_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 1 --reason "Draft ready for final review."
...
thinkingmach pipelines case transition -C "$THINKINGMACH_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 4 --reason "Revised draft ready."
thinkingmach pipelines case review -C "$THINKINGMACH_COMPANY_ID" "$CHANGELOG_CASE" --approve --expected-version 5
thinkingmach pipelines case transition -C "$THINKINGMACH_COMPANY_ID" "$CHANGELOG_CASE" --to published --expected-version 6 --reason "Published after request-changes loop."
```
...
```sh
thinkingmach pipelines case transition -C "$THINKINGMACH_COMPANY_ID" "$TWEET_CASE" --to final_review --expected-version 1 --reason "Blog blocker is now done."
...
```sh
thinkingmach pipelines case rollup \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$RELEASE_CASE_ID" \
  --json
```
...
```sh
thinkingmach pipelines case events \
  -C "$THINKINGMACH_COMPANY_ID" \
  "$CHANGELOG_CASE" \
  --json
```
