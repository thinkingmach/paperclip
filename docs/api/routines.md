...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-ThinkingMach-Signature` + `X-ThinkingMach-Timestamp` header pair matching the trigger's signing mode.
