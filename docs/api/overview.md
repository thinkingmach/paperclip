...
ThinkingMach exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`THINKINGMACH_API_KEY`)
...
- Run audit trail: include `X-ThinkingMach-Run-Id` header on all mutating requests during heartbeats
