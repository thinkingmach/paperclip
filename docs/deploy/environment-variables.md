...
All environment variables that ThinkingMach uses for server configuration.
...
| `THINKINGMACH_BIND` | `loopback` | Reachability preset: `loopback`, `lan`, `tailnet`, or `custom` |
| `THINKINGMACH_BIND_HOST` | (unset) | Required when `THINKINGMACH_BIND=custom` |
| `HOST` | `127.0.0.1` | Legacy host override; prefer `THINKINGMACH_BIND` for new setups |
...
| `THINKINGMACH_DEPLOYMENT_EXPOSURE` | `private` | Exposure policy when deployment mode is `authenticated` |
| `THINKINGMACH_API_URL` | (auto-derived) | ThinkingMach API base URL. When set externally (e.g., via Kubernetes ConfigMap, load balancer, or reverse proxy), the server preserves the value instead of deriving it from the listen host and port. Useful for deployments where the public-facing URL differs from the local bind address. |
| `THINKINGMACH_RUNNER_PUBLIC_URL` | (unset) | Explicit `wss://` base URL used only when a remote `paperclip_runner` target dials ThinkingMach directly. ThinkingMach appends `/api/runner/v1/connect/<runId>`; the reverse proxy must forward WebSocket upgrades for that route. This value is never inferred from request headers. Daytona ignores it and uses provider ingress. |
| `THINKINGMACH_RUNNER_CA_BUNDLE_PATH` | (unset) | Optional PEM CA bundle for direct runner WSS. Platform roots remain enabled. There is no insecure TLS bypass. |
| `THINKINGMACH_RUNNER_REMOTE_BINARY_PATH` | (host build) | Host-local path to a `paperclip-runnerd` artifact built for the remote target OS and architecture. Required when ThinkingMach and the remote sandbox do not share a compatible platform; build metadata and the required transport mode are verified before launch. |
| `THINKINGMACH_RUNNER_REMOTE_CODEX_PATH` | (unset) | Optional host-local path to a Codex executable built for the remote target OS and architecture. For remote Codex-backed runners, ThinkingMach stages and verifies this executable beside `paperclip-runnerd`. |
| `THINKINGMACH_RUNNER_REMOTE_CODEX_NPM_SPEC` | (unset) | Optional pinned npm package spec (for example, `@openai/codex@0.148.0`) installed inside each fresh remote lease when its Codex harness is not baked into the sandbox image. Mutually exclusive with `THINKINGMACH_RUNNER_REMOTE_CODEX_PATH`; ThinkingMach verifies the installed executable before starting `runnerd`. |
| `THINKINGMACH_RUNNER_REMOTE_PROVIDER_PACK_PATH` | (unset) | Host-local path to the immutable provider pack built by `pnpm --filter @thinkingmach/paperclip-runner build:provider-pack`. The pack includes its target-built Node 24.11 runtime, locked production dependencies, OpenCode proxy/executable, and ACPX sidecar. Remote OpenCode and ACPX fail closed without it. A preinstalled pack is accepted only when its complete digested manifest matches this build-owned pack; otherwise ThinkingMach stages this pack into the sandbox. |
| `THINKINGMACH_HIDDEN_SETTINGS` | (unset) | Comma-separated settings surfaces to hide from the UI and floor at the API, for operators hosting ThinkingMach for others (managed cloud, internal shared server). See [Hiding settings surfaces](#hiding-settings-surfaces). |
| `THINKINGMACH_SETTING_DEFAULTS` | (unset) | JSON object replacing the schema default of selected instance settings, for hosting operators. See [Operator setting defaults](#operator-setting-defaults). |
...
Disabling ThinkingMach Runner blocks fresh native starts while persisted native
...
before `PATH`. ThinkingMach verifies runner build metadata, the selected PRP
...
to `THINKINGMACH_RUNNER_REMOTE_BINARY_PATH` and
`THINKINGMACH_RUNNER_REMOTE_CODEX_NPM_SPEC` (or
`THINKINGMACH_RUNNER_REMOTE_CODEX_PATH`) without changing the selected transport.
OpenCode and ACPX instead fall back only to
`THINKINGMACH_RUNNER_REMOTE_PROVIDER_PACK_PATH`; they never start a provider
process on the ThinkingMach host for a remote target.
...
setup sandbox, finish setup, and ThinkingMach captures and promotes the resulting
Daytona snapshot for future leases.

### Hiding settings surfaces

`THINKINGMACH_HIDDEN_SETTINGS` takes keys from the registry in
...
`THINKINGMACH_SETTING_DEFAULTS` takes a JSON object whose fields come from the
...
Example: `THINKINGMACH_SETTING_DEFAULTS='{"feedbackDataSharingPreference":"allowed"}'`
defaults AI feedback sharing to allowed; pairing it with
`instance.general.feedbackDataSharingPreference` in `THINKINGMACH_HIDDEN_SETTINGS`
...
| Variable | Default | Description |
|----------|---------|-------------|
| `THINKINGMACH_SECRETS_MASTER_KEY` | (from file) | 32-byte encryption key (base64/hex/raw) |
| `THINKINGMACH_SECRETS_MASTER_KEY_FILE` | `~/.paperclip/.../secrets/master.key` | Path to key file |
| `THINKINGMACH_SECRETS_STRICT_MODE` | `false` | Require secret refs for sensitive env vars |
...
|----------|-------------|
| `THINKINGMACH_AGENT_ID` | Agent's unique ID |
| `THINKINGMACH_COMPANY_ID` | Company ID |
| `THINKINGMACH_API_URL` | ThinkingMach API base URL (inherits the server-level value; see Server Configuration above) |
| `THINKINGMACH_API_KEY` | Short-lived JWT for API auth |
| `THINKINGMACH_RUN_ID` | Current heartbeat run ID |
| `THINKINGMACH_TASK_ID` | Issue that triggered this wake |
| `THINKINGMACH_WAKE_REASON` | Wake trigger reason |
| `THINKINGMACH_WAKE_COMMENT_ID` | Comment that triggered this wake |
| `THINKINGMACH_APPROVAL_ID` | Resolved approval ID |
| `THINKINGMACH_APPROVAL_STATUS` | Approval decision |
| `THINKINGMACH_LINKED_ISSUE_IDS` | Comma-separated linked issue IDs |
