...
ThinkingMach supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
npx thinkingmach allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, ThinkingMach emits a one-time claim URL at startup:
...
```sh
THINKINGMACH_DEPLOYMENT_MODE=authenticated THINKINGMACH_BIND=lan pnpm thinkingmach run
```
