...
How to get OpenClaw running in a Docker container for local development and testing the ThinkingMach OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

ThinkingMach includes an end-to-end join smoke harness:
...
- probes and prints a ThinkingMach host URL that is reachable from inside OpenClaw Docker
...
- `THINKINGMACH_HOST_PORT` (default `3100`)
- `THINKINGMACH_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your ThinkingMach deployment is `authenticated`, provide auth context:

```bash
THINKINGMACH_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
THINKINGMACH_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host ThinkingMach server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed ThinkingMach URL (typically `http://host.docker.internal:3100`).
- If ThinkingMach rejects the container-visible host with a hostname error, allow it from host:

```bash
npx thinkingmach allowed-hostname host.docker.internal
```

Then restart ThinkingMach and rerun the smoke script.
