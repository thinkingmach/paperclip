...
The **Skills Store** is ThinkingMach's library of reusable skills. A skill is a markdown
...
|---|---|---|
| **The catalog** | A curated, read-only set of skills that ships with ThinkingMach | The `@thinkingmach/skills-catalog` package |
...
- **`bundled`** — first-party ThinkingMach skills (e.g. `issue-triage`, `task-planning`,
  `qa-acceptance`, `wireframe`, `github-pr-workflow`, `doc-maintenance`). These carry the
  reserved `thinkingmach/paperclip/...` key namespace.
...
|---|---|---|
| `catalog` | ThinkingMach / catalog | Installed from the bundled catalog |
...
`ramp` skill is the model: ThinkingMach ships the stable governance wrapper, source
...
a vendored snapshot would go stale, and when ThinkingMach can keep the safety boundary in
the wrapper. For financial, legal, or account-control domains, the wrapper must require
ThinkingMach approvals before spend, incorporation, account authorization, card issuance,
...
Paste a source and ThinkingMach fetches and imports it. Accepted forms include:
...
The catalog is its own publishable package, `@thinkingmach/skills-catalog`:
