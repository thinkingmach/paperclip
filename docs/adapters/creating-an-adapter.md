...
Build a custom adapter to connect ThinkingMach to any agent runtime.
...
| Distribution | Ships with ThinkingMach | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying ThinkingMach's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@thinkingmach/adapter-utils/server-utils`
2. Build environment with `buildThinkingMachEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@thinkingmach/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildThinkingMachEnv(agent)` | `@thinkingmach/adapter-utils/server-utils` | Inject `THINKINGMACH_*` env vars |
| `renderTemplate(tpl, data)` | `@thinkingmach/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@thinkingmach/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@thinkingmach/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in ThinkingMach's source:
...
`format-event.ts` — pretty-prints stdout for `thinkingmach run --watch` using `picocolors`.
...
With these flags set, the ThinkingMach UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no ThinkingMach source changes required.
...
Make ThinkingMach skills discoverable to your agent runtime without writing to the agent's working directory:
