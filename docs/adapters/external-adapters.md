...
| Distribution | Ships with ThinkingMach | Published to npm or linked via `file:` |
| Updates | Requires ThinkingMach release | Independent versioning |
...
- `hermes_local` starts the local Hermes CLI from
  `@thinkingmach/hermes-paperclip-adapter`.
- `hermes_gateway` calls an already-running Hermes API server through
  `@thinkingmach/hermes-paperclip-adapter/gateway`.

The legacy `@thinkingmach/adapter-hermes-gateway` package is a deprecated
...
depend on or link `@thinkingmach/hermes-paperclip-adapter` and declare the type
...
```json
{
  ...
  "dependencies": {
    "@thinkingmach/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^24.0.0",
...
export async function execute(
  ...
  // 2. Build environment with ThinkingMach vars injected
  const env = buildThinkingMachEnv(agent);
...
#### Available Helpers from `@thinkingmach/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildThinkingMachEnv(agent)` | Inject `THINKINGMACH_*` environment variables |
...
Local adapters are symlinked into ThinkingMach's adapter directory. Changes to the source are picked up on server restart.
...
Other ThinkingMach users can then install your adapter by package name from the UI or API.
