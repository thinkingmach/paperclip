...
summary: Ship a custom run-log parser so the ThinkingMach UI renders your adapter's output correctly
---
...
│  ThinkingMach Server  │◄────────────────────────────────│  uiParserCache    │
│  (in-memory)      │                                 └──────────────────┘
└────────┬─────────┘
         │ serves JS to browser
         ▼
┌──────────────────┐   fetch() + eval   ┌──────────────────┐
│  ThinkingMach UI     │─────────────────────→│  parseStdoutLine │
...
The ThinkingMach host checks this field. If the major version is unsupported, the host logs a warning and falls back to the generic parser instead of executing potentially incompatible code.
