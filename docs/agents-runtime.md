...
Audience: Operators setting up and running agents in ThinkingMach

## 1. What this system does

Agents in ThinkingMach do not run continuously.  
...
- `hermes_local`: starts your local `hermes` CLI through `@thinkingmach/hermes-paperclip-adapter`
- `hermes_gateway`: calls an already-running Hermes API server through `@thinkingmach/hermes-paperclip-adapter/gateway`
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), ThinkingMach assumes the CLI is already installed and authenticated on the host machine. For `hermes_gateway`, ThinkingMach assumes the Hermes API server is already running, reachable from the ThinkingMach server, and configured with an API key. The older `@thinkingmach/adapter-hermes-gateway` npm package is only a deprecated compatibility shim; the adapter type remains `hermes_gateway`.
...
ThinkingMach stores session IDs for resumable adapters.
...
ThinkingMach pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. ThinkingMach surfaces this as a warning in environment tests, not a hard error.
