...
Add first-class **documents** to ThinkingMach as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by ThinkingMach workflow/docs.
...
Update the ThinkingMach agent workflow so planning no longer edits the issue description.
