...
The ThinkingMach CLI handles instance setup, diagnostics, and control-plane operations.
...
| Flag | Description |
|------|-------------|
| `--data-dir <path>` | Local ThinkingMach data root (isolates from `~/.paperclip`) |
...
```sh
# Set defaults
npx thinkingmach context set --api-base http://localhost:3100 --company-id <id>
...
Secret operations are available under `thinkingmach secrets`:
