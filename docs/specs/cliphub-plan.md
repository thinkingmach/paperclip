# ClipHub: Marketplace for ThinkingMach Team Configurations
...
> The "app store" for whole-company AI teams — pre-built ThinkingMach configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for ThinkingMach-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | ThinkingMach company (agents, projects, governance) |
...
A complete ThinkingMach company configuration:
...
Single-agent configurations designed to plug into a ThinkingMach org:
...
Portable skill files that any ThinkingMach agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with ThinkingMach's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum ThinkingMach version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target ThinkingMach company
...
| `POST` | `/api/listings/:id/install` | Install to ThinkingMach company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target ThinkingMach company (or create new)
  → ClipHub API calls ThinkingMach API to:
  ...
  → Redirect to ThinkingMach dashboard with new team running
```
...
### 5.3 Creator: Export from ThinkingMach → Publish

```
Running ThinkingMach company → "Export as Blueprint" (CLI or UI)
  → ThinkingMach exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (ThinkingMach brand blue/purple)
...
| Install flow | One-click deploy to ThinkingMach company |
...
{
  "targetCompanyId": "uuid",         // Existing ThinkingMach company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as ThinkingMach
- **Backend**: Node.js API (or extend ThinkingMach server)
- **Database**: Postgres (can share ThinkingMach's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with ThinkingMach auth (or OAuth2)

### 9.2 Integration with ThinkingMach

ClipHub can be:
- **Option A**: A separate app that calls ThinkingMach's API to install blueprints
- **Option B**: A built-in section of the ThinkingMach UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing ThinkingMach UI and API.
...
- [ ] Install flow: blueprint → ThinkingMach company
...
- [ ] "Export from ThinkingMach" CLI command
