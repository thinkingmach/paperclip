...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only ThinkingMach.
...
- be useful outside ThinkingMach
...
Example ThinkingMach extension:
...
- ThinkingMach must not require extra top-level fields for skill validity
- ThinkingMach-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
For ThinkingMach, the preferred fidelity extension is:
...
- issue/task ThinkingMach-only metadata
...
- ThinkingMach tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested ThinkingMach shape:
...
Additional rules for ThinkingMach exporters:
...
- omit empty and default-valued ThinkingMach fields when possible
...
- ThinkingMach exporters should emit `.paperclip.yaml` as a sidecar by default
...
## 19. ThinkingMach Mapping

ThinkingMach can map this spec to its runtime model like this:
...
- ThinkingMach extension:
  - `.paperclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other ThinkingMach-specific fidelity

Inline ThinkingMach-only metadata that must live inside a shared markdown file should use:

- `metadata.paperclip`

That keeps the base format broader than ThinkingMach.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only ThinkingMach.

## 20. Cutover

ThinkingMach should cut over to this markdown-first package model as the primary portability format.

`paperclip.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For ThinkingMach, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
