/**
 * esbuild configuration for building the thinkingmach CLI for npm.
 *
 * Bundles all workspace packages (@thinkingmach/*) into a single file.
...
// to npm and resolved at runtime (e.g. @thinkingmach/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@thinkingmach/") && !bundledCliNpmDependencies.has(name)) {
      externals.add(name);
    }
  }
