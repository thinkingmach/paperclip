...
import { isSupportedNodeVersion, MINIMUM_NODE_VERSION } from "@thinkingmach/shared/node-version";
import {
  addManagedPathBlock,
...
const DEFAULT_GITHUB_REPO = "thinkingmach/paperclip";
  ...
  const visit = (packageName: string): void => {
    ...
    for (const section of ["dependencies", "optionalDependencies", "peerDependencies"] as const) {
      ...
      for (const dependencyName of Object.keys(dependencies)) {
        if (dependencyName.startsWith("@thinkingmach/")) visit(dependencyName);
      }
    }
...
export async function resolvePublishedVersion(spec: string, runCommand: CommandRunner): Promise<string> {
  const result = await runCommand(
    "npm",
    ["view", `thinkingmach@${spec}`, "version", "--json", `--registry=${PUBLIC_NPM_REGISTRY}`],
    { maxBuffer: 1024 * 1024 },
  );
...
async function runGitHubCurl(
  ...
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), "thinkingmach-gh-"));
...
export async function resolveGitHubRef(repo: string, ref: string, runCommand: CommandRunner): Promise<string> {
  const result = await runGitHubCurl(["--fail", "--silent", "--show-error", "--location", "--header", "Accept: application/vnd.github+json", "--header", "User-Agent: thinkingmach-install", `https://api.github.com/repos/${repo}/commits/${encodeURIComponent(ref)}`], runCommand, { maxBuffer: 4 * 1024 * 1024 });
  let sha: unknown;
...
function payloadEntrypoint(payloadPath: string): string {
  return path.join(payloadPath, "node_modules", "thinkingmach", "dist", "index.js");
}
...
export async function installGitPayload(repo: string, sha: string, runCommand: CommandRunner, paths = resolveInstallStorePaths()): Promise<{ payloadPath: string; reused: boolean; version: string }> {
  ...
  if (fs.existsSync(payloadPath)) {
    const metadata = JSON.parse(fs.readFileSync(path.join(payloadPath, "node_modules", "thinkingmach", "package.json"), "utf8")) as { version: string };
  ...
  try {
    ...
    await runCommand("corepack", ["pnpm", "-r", "--filter", "@thinkingmach/server...", "--if-present", "run", "build"], { cwd: checkoutPath, env: buildEnv(), maxBuffer: 32 * 1024 * 1024 });
    ...
    for (const [index, workspacePackage] of workspacePackages.entries()) {
      ...
      if (bundledDependencies.length > 0) {
        ...
        await runCommand("corepack", ["pnpm", "--dir", workspacePackage.dir, "pack", "--pack-destination", stagingRoot], { cwd: checkoutPath, env: buildEnv({ THINKINGMACH_RELEASE_REUSE_UI_DIST: "1" }), maxBuffer: 32 * 1024 * 1024 });
      }
    }
    ...
    const cliTarball = tarballs.find((entry) => entry === `thinkingmach-${metadata.version}.tgz`);
...
async function ensureShimOnPath(options: InstallOptions): Promise<void> {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY || !rcPath) {
    console.log(pc.yellow(`Add ThinkingMach to PATH for this shell:\n  ${manualInstruction}`));
    return;
  }
...
export async function installCommand(
  ...
  if (gitRequest) {
    ...
    console.log(pc.green(`${installed.reused ? "Activated cached" : "Installed"} thinkingmach git payload ${sha.slice(0, 12)}.`));
    return;
  }
  const request = resolveNpmInstallRequest(options);
  console.log(`Resolving thinkingmach@${request.spec} from ${PUBLIC_NPM_REGISTRY}...`);
  const version = await resolvePublishedVersion(request.spec, runCommand);
  console.log(`Installing thinkingmach@${version}...`);
  ...
  console.log(pc.green(`${installed.reused ? "Activated cached" : "Installed"} thinkingmach ${version} (${request.channel}).`));
  console.log(pc.dim(`Payload: ${installed.payloadPath}`));
  console.log(`Run ${pc.cyan("thinkingmach --version")} to verify the managed install.`);
}
