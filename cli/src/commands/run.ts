...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" thinkingmach run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("thinkingmach onboard")} once, then retry ${pc.cyan("thinkingmach run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting ThinkingMach server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.THINKINGMACH_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@thinkingmach/server/src/index.ts")) {
    process.env.THINKINGMACH_UI_DEV_MIDDLEWARE = "true";
  }
}
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the ThinkingMach dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @thinkingmach/server package
  try {
    const mod = await import("@thinkingmach/server");
    return await startServerFromModule(mod, "@thinkingmach/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@thinkingmach/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a ThinkingMach server entrypoint.\n` +
          `Tried: ${devEntry}, @thinkingmach/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `ThinkingMach server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: ThinkingMachConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`ThinkingMach server entrypoint did not export startServer(): ${label}`);
  }
