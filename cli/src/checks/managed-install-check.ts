...
export function nodeRuntimeCheck(): CheckResult {
  return isSupportedNodeVersion(process.versions.node)
    ...
    : {
        ...
        repairHint: `Install Node.js ${MINIMUM_NODE_VERSION} or newer before installing or running ThinkingMach`,
      };
}
    ...
    return [
      {
        ...
        repairHint: "Re-run `thinkingmach install` to rebuild the managed install metadata",
      },
    ];
  }
    ...
    return [
      {
        ...
        repairHint: "Re-run `thinkingmach install`",
      },
    ];
  }
  ...
  results.push(
    payloadExists && currentMatches
      ...
      : {
          ...
          repairHint: "Re-run `thinkingmach install` or roll back to a retained payload",
        },
  );
  ...
  results.push(
    orphaned.length === 0
      ...
      : {
          ...
          repairHint: "A successful `thinkingmach update` prunes unretained payloads",
        },
  );
