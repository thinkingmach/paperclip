...
export async function databaseCheck(config: ThinkingMachConfig, configPath?: string): Promise<CheckResult> {
  if (config.database.mode === "postgres") {
    if (!config.database.connectionString) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `thinkingmach configure --section database`",
      };
    }
  ...
  if (config.database.mode === "embedded-postgres") {
    ...
    // flag: this is what happens when THINKINGMACH_HOME / THINKINGMACH_IN_WORKTREE leak
    ...
    // ephemeral/CI instances that don't set THINKINGMACH_IN_WORKTREE are not flagged.)
    // Check BEFORE creating the dir so we don't bootstrap the very temp location
    // we're warning about.
    if (isInsideOsTmpDir(dataDir) && process.env.THINKINGMACH_IN_WORKTREE === "true") {
      return {
        name: "Database",
        status: "warn",
        message:
          `Embedded PostgreSQL data dir is inside the OS temp directory (${dataDir}) ` +
          "while running in worktree mode (THINKINGMACH_IN_WORKTREE=true). Data stored here is " +
          "ephemeral and will be lost on reboot or a temp cleanup. If this is your primary " +
          "instance, THINKINGMACH_HOME / THINKINGMACH_IN_WORKTREE likely leaked into its environment, " +
          "pointing it at a throwaway worktree home instead of your real data.",
        canRepair: false,
        repairHint:
          "If this is the primary instance, unset THINKINGMACH_HOME and THINKINGMACH_IN_WORKTREE " +
