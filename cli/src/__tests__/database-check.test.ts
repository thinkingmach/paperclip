...
const ORIGINAL_IN_WORKTREE = process.env.THINKINGMACH_IN_WORKTREE;
...
function embeddedConfig(dataDir: string): ThinkingMachConfig {
  return {
    database: {
  ...
  } as unknown as ThinkingMachConfig;
}

afterEach(() => {
  vi.restoreAllMocks();
  if (ORIGINAL_IN_WORKTREE === undefined) delete process.env.THINKINGMACH_IN_WORKTREE;
  else process.env.THINKINGMACH_IN_WORKTREE = ORIGINAL_IN_WORKTREE;
...
describe("databaseCheck — embedded postgres temp-dir guard", () => {
  ...
  it("warns when a worktree-mode data dir lives inside the OS temp directory", async () => {
    ...
    expect(result.repairHint).toContain("THINKINGMACH_HOME");
