...
export class ThinkingMachApiClient {
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the ThinkingMach API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the ThinkingMach server is not running, the configured URL is wrong, or the request is being blocked before it reaches ThinkingMach.",
    "",
    "Try:",
    "- Start ThinkingMach with `pnpm dev` (from a source checkout) or `npx thinkingmach run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If ThinkingMach is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`THINKINGMACH_API_URL\`.`,
  );
  return lines.join("\n");
}
