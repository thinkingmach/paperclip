...
export function resolveBoardAuthStorePath(overridePath?: string): string {
  ...
  if (process.env.THINKINGMACH_AUTH_STORE?.trim()) return path.resolve(process.env.THINKINGMACH_AUTH_STORE.trim());
...
export async function loginBoardCli(params: {
  ...
  const challenge = await requestJson<CreateChallengeResponse>(createUrl, {
    ...
    body: JSON.stringify({
      command,
      clientName: params.clientName?.trim() || "thinkingmach cli",
      requestedAccess: params.requestedAccess,
      requestedCompanyId: params.requestedCompanyId?.trim() || null,
    }),
  });

  const publicBase = params.publicBaseUrl?.trim() || process.env.THINKINGMACH_PUBLIC_URL?.trim();
  ...
  const wantBrowser = params.openBrowser !== false && !isTruthyEnv(process.env.THINKINGMACH_NO_BROWSER);
