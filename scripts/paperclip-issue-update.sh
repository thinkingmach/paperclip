...
Examples:
  scripts/paperclip-issue-update.sh --issue-id "$THINKINGMACH_TASK_ID" --status in_progress <<'MD'
  ...
  scripts/paperclip-issue-update.sh --issue-id "$THINKINGMACH_TASK_ID" --status done --dry-run <<'MD'
  Done
...
if [[ -z "$issue_id" ]]; then
  printf 'Missing issue id. Pass --issue-id or set THINKINGMACH_TASK_ID.\n' >&2
  exit 1
fi
...
while :; do
  ...
  response="$(
    curl -sS -m 30 -X PATCH \
      "$THINKINGMACH_API_URL/api/issues/$issue_id" \
