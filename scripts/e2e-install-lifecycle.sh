#!/usr/bin/env bash
# End-to-end proof of the thinkingmach managed install lifecycle on a CLEAN machine.
#
...
#
# Env knobs:
#   E2E_REPO          GitHub repo to install from (default: thinkingmach/paperclip)
...
# A clean environment: no inherited ThinkingMach or build-mode state.
...
# bootstrap simulates what `npx thinkingmach@<channel> install` will run post-release:
...
BOOTSTRAP_CLI="$HOME/e2e-bootstrap-cli/node_modules/thinkingmach/dist/index.js"
...
if [ "${E2E_SKIP_NPM:-0}" != "1" ]; then
  ...
  [ -x "$SHIM" ] && pass "2b shim created at ~/.local/bin/thinkingmach" || fail_ "2b shim created"
  ...
  [ -n "$NPM_VERSION" ] && pass "2e shim runs: thinkingmach --version = $NPM_VERSION" || fail_ "2e shim runs thinkingmach --version"
else
