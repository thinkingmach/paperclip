...
# The CLI derives the source from THINKINGMACH_WORKSPACE_BASE_CWD, which the control
...
if [[ ! -e "$base_cwd/.paperclip/config.json" && ! -L "$base_cwd/.paperclip/config.json" ]]; then
  ...
  # THINKINGMACH_CONFIG at the target. Naming the target as its own source is never
...
run_ensure_seeded() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm thinkingmach --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd" &&
        pnpm thinkingmach worktree ensure-seeded --config "$worktree_config_path" ${source_config_args[@]+"${source_config_args[@]}"}
    )
    return
  fi
  ...
  if command -v thinkingmach >/dev/null 2>&1; then
    (
      cd "$worktree_cwd" &&
        thinkingmach worktree ensure-seeded --config "$worktree_config_path" ${source_config_args[@]+"${source_config_args[@]}"}
    )
    return
  fi
...
if run_ensure_seeded; then
  ...
  if [[ "$exit_code" -eq 127 ]]; then
    echo "No usable thinkingmach CLI found; cannot seed the worktree database." >&2
  fi
  exit "$exit_code"
fi
