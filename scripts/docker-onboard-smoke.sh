...
THINKINGMACH_VERSION="${THINKINGMACH_VERSION:-latest}"
...
THINKINGMACH_DEPLOYMENT_MODE="${THINKINGMACH_DEPLOYMENT_MODE:-authenticated}"
THINKINGMACH_DEPLOYMENT_EXPOSURE="${THINKINGMACH_DEPLOYMENT_EXPOSURE:-private}"
THINKINGMACH_PUBLIC_URL="${THINKINGMACH_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
# Seconds to wait for /api/health after the container starts. The container
# cold-installs thinkingmach from npm and initializes embedded postgres before
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_THINKINGMACH_VERSION=%q\n' "$THINKINGMACH_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e THINKINGMACH_DEPLOYMENT_MODE="$THINKINGMACH_DEPLOYMENT_MODE" \
      -e THINKINGMACH_DEPLOYMENT_EXPOSURE="$THINKINGMACH_DEPLOYMENT_EXPOSURE" \
      -e THINKINGMACH_PUBLIC_URL="$THINKINGMACH_PUBLIC_URL" \
      -e THINKINGMACH_HOME="/paperclip" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "thinkingmach@${THINKINGMACH_VERSION}" auth bootstrap-ceo --data-dir "$THINKINGMACH_HOME" --base-url "$THINKINGMACH_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
post_json_with_cookies() {
  ...
  curl -sS \
    ...
    -H "Origin: $THINKINGMACH_PUBLIC_URL" \
    -X POST \
    "$url" \
    --data "$body"
}
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$THINKINGMACH_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$THINKINGMACH_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$THINKINGMACH_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$THINKINGMACH_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$THINKINGMACH_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$THINKINGMACH_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg THINKINGMACH_VERSION="$THINKINGMACH_VERSION" \
...
echo "    Public URL: $THINKINGMACH_PUBLIC_URL"
...
echo "    Deployment: $THINKINGMACH_DEPLOYMENT_MODE/$THINKINGMACH_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$THINKINGMACH_PUBLIC_URL/api/health" "$SMOKE_READY_TIMEOUT_SECONDS" 1; then
  echo "Smoke bootstrap failed: server did not become ready at $THINKINGMACH_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$THINKINGMACH_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $THINKINGMACH_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
