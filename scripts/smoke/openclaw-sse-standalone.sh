...
THINKINGMACH_RUN_ID="${THINKINGMACH_RUN_ID:-smoke-run-$(date +%s)}"
THINKINGMACH_AGENT_ID="${THINKINGMACH_AGENT_ID:-openclaw-smoke-agent}"
THINKINGMACH_COMPANY_ID="${THINKINGMACH_COMPANY_ID:-openclaw-smoke-company}"
THINKINGMACH_API_URL="${THINKINGMACH_API_URL:-http://localhost:3100}"
THINKINGMACH_TASK_ID="${THINKINGMACH_TASK_ID:-openclaw-smoke-task}"
THINKINGMACH_WAKE_REASON="${THINKINGMACH_WAKE_REASON:-openclaw_smoke_test}"
THINKINGMACH_WAKE_COMMENT_ID="${THINKINGMACH_WAKE_COMMENT_ID:-}"
THINKINGMACH_APPROVAL_ID="${THINKINGMACH_APPROVAL_ID:-}"
THINKINGMACH_APPROVAL_STATUS="${THINKINGMACH_APPROVAL_STATUS:-}"
THINKINGMACH_LINKED_ISSUE_IDS="${THINKINGMACH_LINKED_ISSUE_IDS:-}"
...
THINKINGMACH_RUN_ID=${THINKINGMACH_RUN_ID}
THINKINGMACH_AGENT_ID=${THINKINGMACH_AGENT_ID}
THINKINGMACH_COMPANY_ID=${THINKINGMACH_COMPANY_ID}
THINKINGMACH_API_URL=${THINKINGMACH_API_URL}
THINKINGMACH_TASK_ID=${THINKINGMACH_TASK_ID}
THINKINGMACH_WAKE_REASON=${THINKINGMACH_WAKE_REASON}
THINKINGMACH_WAKE_COMMENT_ID=${THINKINGMACH_WAKE_COMMENT_ID}
THINKINGMACH_APPROVAL_ID=${THINKINGMACH_APPROVAL_ID}
THINKINGMACH_APPROVAL_STATUS=${THINKINGMACH_APPROVAL_STATUS}
THINKINGMACH_LINKED_ISSUE_IDS=${THINKINGMACH_LINKED_ISSUE_IDS}

Run your ThinkingMach heartbeat procedure now.
EOF
...
PAYLOAD="$(jq -nc \
  ...
  --arg agentId "$THINKINGMACH_AGENT_ID" \
  --arg companyId "$THINKINGMACH_COMPANY_ID" \
  --arg apiUrl "$THINKINGMACH_API_URL" \
  --arg taskId "$THINKINGMACH_TASK_ID" \
  --arg wakeReason "$THINKINGMACH_WAKE_REASON" \
  --arg wakeCommentId "$THINKINGMACH_WAKE_COMMENT_ID" \
  --arg approvalId "$THINKINGMACH_APPROVAL_ID" \
  --arg approvalStatus "$THINKINGMACH_APPROVAL_STATUS" \
  --arg linkedIssueIds "$THINKINGMACH_LINKED_ISSUE_IDS" \
  '{
    model: $model,
    user: $user,
    input: $text,
    ...
    metadata: {
      ...
      THINKINGMACH_AGENT_ID: $agentId,
      THINKINGMACH_COMPANY_ID: $companyId,
      THINKINGMACH_API_URL: $apiUrl,
      THINKINGMACH_TASK_ID: $taskId,
      THINKINGMACH_WAKE_REASON: $wakeReason,
      THINKINGMACH_WAKE_COMMENT_ID: $wakeCommentId,
      THINKINGMACH_APPROVAL_ID: $approvalId,
      THINKINGMACH_APPROVAL_STATUS: $approvalStatus,
      THINKINGMACH_LINKED_ISSUE_IDS: $linkedIssueIds,
...
args=(
  ...
  -H "x-openclaw-session-key: paperclip:run:${THINKINGMACH_RUN_ID}"
  -D "$headers_file"
  -o "$body_file"
  --data "$PAYLOAD"
  "$OPENCLAW_URL"
